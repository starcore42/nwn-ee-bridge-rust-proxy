...
- 2026-07-17 live gate and terminal handoff classification: the prior gameplay
  ...
  field topology and widths. Field values stay independent so this class can
  narrow the owner trace without claiming a replay. The source bytes are
  already exhausted at `245..245`; a second stock `U` row would require 10
  header bytes, but zero are available, so
  `second_stock_row_dispatch_possible=false`. This is bounded
  diagnostic evidence, not writer ownership: `writer_replay_proven=false`,
  ownership remains unknown, and claim, rewrite, and fragment trimming remain
  false.
...
- 2026-07-17 alternating door/placeable live-object fragment ownership:
  ...
  Fresh current-code gate capture, rechecked at `2026-07-17T16:17+10:00`
  (about 17h58m49s old),
  ...
  The version-4 terminal artifact now reduces those walks to typed field rows
  ...
  The same artifact now classifies the end-aligned `63..76` interpretation as
  a reused-record reader counterfactual only when its object, masks, and
  complete ordered relative field topology and widths match the stock walk,
  its start is contiguous with the stock end, it ends at the declared fragment
  boundary,
  and the byte cursor is exhausted. A second stock `U` dispatch requires 10
  header bytes but has zero available. The result therefore records
  `second_stock_row_dispatch_possible=false`, `writer_replay_proven=false`,
  and unknown writer ownership; it cannot authorize a claim, rewrite, trim,
  or cursor move.
  ...
  and zero terminal live-object residuals on isolated ports 65121/65133; stderr
  was empty, and all 29 focused `tail9` tests passed. An earlier attempt exposed
  and rejected a stack-size regression
