...
fn finish_ack_delivery(
    ...
    for ack_sequence in &ack_sequences {
        if owner.acknowledges_server_sources() {
            ...
            if !retired_sources.is_empty() {
                state.sequence.server_sequence_epoch_source_floor =
                    server_replay::output_retirement_floor(&state.server_reliable_slots);
            ...
            retired_slots = retired_slots.saturating_add(retired_sources.len());
        }
    }
    tracing::trace!(
        owner = owner.as_str(),
...
/// Finalize one HG-facing reliable receive slot and publish its cumulative ACK.
///
/// Diamond `sub_5F3940` lines 751605-751673 and EE `FrameReceive` lines
/// 879029-879088 clear each contiguous receive slot and advance the receive
/// frontiers immediately after dispatch. Their destination send windows are
/// independent. Queue the exact type-1 ACK only after proxy2's outer strict
/// result commits the same source, so rejected CNW/EE output cannot advance HG.
fn finish_server_source_dispatch(state: &mut SessionState, accepted: bool) {
    let Some(committed) =
        server_replay::finish_source_dispatch(&mut state.server_reliable_slots, accepted)
    else {
        return;
    };
    if let Err(err) = local_ack::queue_consumed_server_frame_ack(
        state,
        committed.sequence,
        local_ack::COMMITTED_SERVER_RECEIVE_ACK_REASON,
    ) {
        tracing::error!(
            error = %err,
            sequence = committed.sequence,
            origin_generation = committed.origin_generation,
            "strict-committed HG receive frontier could not publish its cumulative ACK"
        );
    }
}
...
pub fn translate_client_to_server(bytes: &[u8], state: &mut SessionState) -> anyhow::Result<Emit> {
    ...
    match client_replay::prepare_source_dispatch(
        &mut state.client_reliable_replays,
        &prepared_source,
    )? {
        client_replay::ClientReliableDispatchAdmission::Excluded
        | client_replay::ClientReliableDispatchAdmission::Ready(_) => {}
        client_replay::ClientReliableDispatchAdmission::Future { key, expected } => {
            tracing::info!(
                sequence = key.sequence,
                origin_generation = key.origin_generation,
                ...
                ack_sequence = view.ack_sequence,
                "client reliable source buffered behind missing predecessor without entering engine-facing semantics"
            );
            return Ok(Emit::Consumed);
        }
    }
...
fn queue_proxy_owned_ack_for_committed_client_frontier(
    state: &mut SessionState,
    ack_sequence: u16,
) -> anyhow::Result<()> {
    client_ack::queue_committed_receive_ack(&mut state.client_ack.pending, ack_sequence);
    Ok(())
}
...
pub fn translate_server_to_client(bytes: &[u8], state: &mut SessionState) -> anyhow::Result<Emit> {
    ...
    let outside_window_ack_sequence = matches!(
        ...
        server_replay::PreparedServerReliableSource::OutputReplay(_)
            | server_replay::PreparedServerReliableSource::OutputConflict(_)
    ...
    match prepared_server_source {
        ...
        server_replay::PreparedServerReliableSource::OutputConflict(key) => {
            observe_validated_server_source_ack(state, source_ack_sequence);
            tracing::warn!(
                sequence = key.sequence,
                origin_generation = key.origin_generation,
                ...
                "server reliable replay rejected because its EE-output-owned source carried different immutable bytes"
            );
            return Ok(Emit::Drop);
        }
        ...
        server_replay::PreparedServerReliableSource::OutsideWindow(key) => {
            ...
            tracing::warn!(
                ...
                retained_output_sources = diagnostic.retained_output_sources,
    ...
    let emit = match translate_server_to_client_inner(bytes, state) {
        ...
        Err(err) => {
            ...
            finish_server_source_dispatch(state, false);
            return Err(err);
        }
    };
    ...
    if matches!(&emit, Emit::Drop) {
        ...
        if validation_owned_effects {
            ...
            finish_server_source_dispatch(state, true);
        }
    }
    Ok(emit)
}
...
/// and the normal outer strict validator. Merely taking the packet does not
/// remove its active receive slot: strict dispatch acceptance releases that
/// slot into independent output-replay ownership, while rejection waits for a
/// real exact retransmit.
...
/// Take one raw client source that was stored behind a receive gap and became
/// the exact semantic frontier after its predecessor committed.
///
/// The caller feeds this plaintext frame back through the ordinary client
/// translator and outer strict validator. Merely taking it does not release
...
pub(super) fn finish_pending_client_drain_emit_validation(
    ...
    let committed_slots = diamond_send_window::finish(
        &mut state.diamond_client_send_window,
        diamond_send_window::DiamondClientSendOwner::PendingClientDrain,
        accepted,
    );
    if accepted
        && committed_slots.is_some()
        && inventory_equipment::observe_staged_client_gui_status_close_committed(state)
    {
...
pub(super) fn finish_client_to_server_emit_validation_outcomes(
    ...
    let diamond_output_committed = diamond_send_window::finish(
        &mut state.diamond_client_send_window,
        diamond_send_window::DiamondClientSendOwner::DirectClient,
        ack_output_accepted,
    )
    .is_some();
    if effects_accepted && !diamond_output_committed {
        tracing::warn!(
            ack_output_accepted,
            "strict client effects stayed uncommitted because Diamond output ownership did not commit"
        );
    }
    finish_client_to_server_effect_validation(
        state,
        effects_accepted && diamond_output_committed,
        ack_output_accepted,
    );
    finish_ack_delivery(
        state,
...
fn finish_client_to_server_effect_validation(
    ...
    if effects_accepted {
        ...
        let committed_source = token
            ...
            .and_then(|key| {
                client_replay::release_committed_prefix(&mut state.client_reliable_replays, key)
            });
        if let Some(committed) = committed_source {
            if let Err(err) =
                queue_proxy_owned_ack_for_committed_client_frontier(state, committed.sequence)
            {
                tracing::error!(
                    error = %err,
                    sequence = committed.sequence,
                    origin_generation = committed.origin_generation,
                    "strict-committed client receive frontier could not publish its cumulative ACK"
                );
            }
        }
...
pub(super) fn finish_server_to_client_emit_validation_outcomes(
    ...
    finish_server_source_dispatch(state, effects_committed);
    finish_ack_delivery(
        state,
        ack_delivery::AckDeliveryOwner::DirectServer,
        ack_output_accepted,
    ...
    match prepared_server_source {
        ...
        server_replay::PreparedServerReliableSource::OutputConflict(key) => {
            ...
            tracing::warn!(
                ...
                ack_sequence = view.ack_sequence,
                receive_start = state.server_reliable_slots.receive_start,
                "server reliable replay rejected before route selection because its EE-output-owned source carried different immutable bytes"
            );
            return Ok(Emit::Drop);
        }
        server_replay::PreparedServerReliableSource::OutputReplay(key) => {
...
pub(super) fn apply_verified_server_semantic_side_effects(
    ...
    inventory_equipment::observe_client_gui_status_transport_boundary(
        state,
        frame.sequence,
    ...
    for observation in &frame.live_object_inventory_materializations {
        let recorded =
            inventory_equipment::maybe_record_client_gui_status_live_object_frame_response(
                ...
                observation.area_client_area_packets,
...
impl CurrentPlayerKnownVisibleEquipment {
    ...
    /// Choose a deterministic exact-object probe participant from the
    /// Status-authorized partial P/5 view. `known_visible_equipment_for_owner`
    /// returns BTreeMap order, so the lowest visible selector wins. Diamond
    /// `sub_448E30`/`sub_4C7A50` and EE `sub_14077FE10`/`sub_1407FFF10`
    /// establish that this selector belongs to the P/5 appearance namespace;
    /// it is provenance only and must never be translated into an Inventory
    /// response slot. Requiring a currently proven exact object keeps compact
    /// aliases and area-reset tombstones out of the engine-facing payload.
    fn first_exact_proven_item(
        &self,
        object_registry: &semantic::ObjectRegistry,
    ) -> Option<(u32, u32)> {
        self.slots.iter().find_map(|(visible_slot, object_id)| {
            matches!(
                object_registry.inventory_item_object_status(*object_id),
                semantic::InventoryItemObjectStatus::Proven(_)
            )
            .then_some((*visible_slot, *object_id))
        })
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum InventoryEquipToggleRecommendationSource {
    ConfirmedServerInventoryEquipClaim,
    StatusAuthorizedCurrentPlayerVisibleEquipment,
}

impl InventoryEquipToggleRecommendationSource {
    fn as_str(self) -> &'static str {
        match self {
            Self::ConfirmedServerInventoryEquipClaim => {
                "confirmed_current_player_inventory_server_equip_claim"
            }
            Self::StatusAuthorizedCurrentPlayerVisibleEquipment => {
                "status_authorized_current_player_visible_equipment"
            }
        }
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct InventoryEquipToggleRecommendationCandidate {
    object_id: u32,
    update_index: u64,
    server_inventory_minor: u8,
    alternate_inventory_context: bool,
    server_inventory_equip_slot: u32,
}
...
fn augment_quickbar_item_refresh_hint_with_bridge_output_and_protocol_state(
    ...
    let first_client_gui_status_response_observation = client_gui_status_response_observations
        ...
        .copied()
        .unwrap_or_default();
    let last_client_gui_status_response_observation = client_gui_status_response_observations
        .back()
        .copied()
        .unwrap_or_default();
    let same_client_gui_status_response_authority =
        |left: &state::InventoryEquipmentBridgeClientGuiStatusResponseObservation,
         right: &state::InventoryEquipmentBridgeClientGuiStatusResponseObservation| {
            left.area_client_area_packets == right.area_client_area_packets
                && left.control_epoch == right.control_epoch
                && left.current_player_object_id == right.current_player_object_id
        };
    let client_gui_status_response_observation_authority_contexts =
        client_gui_status_response_observations
            ...
            .filter(|(index, observation)| {
                !client_gui_status_response_observations
                    .iter()
                    .take(*index)
                    .any(|prior| same_client_gui_status_response_authority(prior, observation))
            })
            .count();
    let client_gui_status_response_observation_forwarded_units =
        client_gui_status_response_observations
            .iter()
            .filter(|observation| observation.forwarded_request)
            .count();
    let client_gui_status_response_observation_units_after_response_completion =
        client_gui_status_response_observations
            .iter()
            .filter(|observation| observation.response_window_complete_before_observation)
            .count();
    let client_gui_status_response_observation_inventory_records =
        client_gui_status_response_observations
            .iter()
            .fold(0u64, |records, observation| {
                records.saturating_add(u64::from(observation.inventory_records))
            });
    let client_gui_status_response_observation_owner_units =
        client_gui_status_response_observations
            .iter()
            .filter(|observation| observation.current_player_inventory_records != 0)
            .count();
    let client_gui_status_response_observation_pre_completion_owner_units =
        client_gui_status_response_observations
            .iter()
            .filter(|observation| {
            ...
            .count();
    let client_gui_status_response_observation_owner_records =
        client_gui_status_response_observations
            .iter()
            .fold(0u64, |records, observation| {
                records.saturating_add(u64::from(observation.current_player_inventory_records))
            });
    let client_gui_status_response_observation_owner_mask_union =
        client_gui_status_response_observations
            .iter()
            .fold(0u16, |mask, observation| {
                mask | observation.current_player_inventory_mask_union
            });
    let client_gui_status_response_observation_materialization_units =
        client_gui_status_response_observations
            .iter()
            .filter(|observation| observation.materialized_item_object_ids != 0)
            .count();
    let client_gui_status_response_observation_materialized_item_object_ids =
        client_gui_status_response_observations
            .iter()
            .fold(0usize, |object_ids, observation| {
                object_ids.saturating_add(observation.materialized_item_object_ids)
            });
    let client_gui_status_response_observation_queued_candidate_units =
        client_gui_status_response_observations
            .iter()
            .filter(|observation| observation.materialized_item_object_ids_contain_queued_candidate)
            .count();
    let client_gui_status_response_observation_same_unit_owner_materialization_units =
        client_gui_status_response_observations
            .iter()
            .filter(|observation| {
                observation.current_player_inventory_records != 0
                    && observation.materialized_item_object_ids != 0
            })
            .count();
    let client_gui_status_response_observation_owner_only_units =
        client_gui_status_response_observations
            .iter()
            .filter(|observation| {
                observation.current_player_inventory_records != 0
            ...
            .count();
    let client_gui_status_response_observation_materialization_only_units =
        client_gui_status_response_observations
            .iter()
            .filter(|observation| {
                observation.current_player_inventory_records == 0
            ...
            .count();
    // The semantic reducer emits one observation per wire-ordered LiveObject
    // unit. Multiple observations can share one M frame or one deflated
    // reassembly, so correlate deque positions as units, not transport frames.
    // Authority must also match: ObjControl or area changes inside one gameplay
    // stream cannot manufacture a Status owner/materialization relationship.
    let mut cross_unit_owner_materialization_pair = None;
    let mut same_unit_owner_materialization_pair = None;
    for (owner_index, owner) in client_gui_status_response_observations.iter().enumerate() {
        if owner.current_player_inventory_records == 0
            || owner.response_window_complete_before_observation
        {
            continue;
        }
        for (materialization_index, materialization) in
            client_gui_status_response_observations.iter().enumerate()
        {
            if materialization.materialized_item_object_ids == 0
                || !same_client_gui_status_response_authority(owner, materialization)
            {
                continue;
            }
            if owner_index == materialization_index {
                same_unit_owner_materialization_pair
                    .get_or_insert((owner_index, materialization_index));
            } else {
                cross_unit_owner_materialization_pair
                    .get_or_insert((owner_index, materialization_index));
            }
        }
    }
    let client_gui_status_response_observation_cross_unit_owner_materialization =
        cross_unit_owner_materialization_pair.is_some();
    let correlated_owner_materialization_pair =
        cross_unit_owner_materialization_pair.or(same_unit_owner_materialization_pair);
    let client_gui_status_response_observation_authority_correlated_owner_materialization =
        correlated_owner_materialization_pair.is_some();
    let client_gui_status_response_observation_correlated_pair_includes_post_completion_unit =
        correlated_owner_materialization_pair.is_some_and(|(owner, materialization)| {
            client_gui_status_response_observations[owner]
                .response_window_complete_before_observation
                || client_gui_status_response_observations[materialization]
                    .response_window_complete_before_observation
        });
    let client_gui_status_response_observation_authority_mismatch_only =
        client_gui_status_response_observation_pre_completion_owner_units != 0
            && client_gui_status_response_observation_materialization_units != 0
            && correlated_owner_materialization_pair.is_none();
    let client_gui_status_response_observation_missing_pre_completion_owner_anchor =
        client_gui_status_response_observation_owner_units != 0
            && client_gui_status_response_observation_materialization_units != 0
            && client_gui_status_response_observation_pre_completion_owner_units == 0
            && client_gui_status_response_observation_units_after_response_completion != 0;
    let client_gui_status_response_observation_owner_materialization_chronology =
        match correlated_owner_materialization_pair {
            Some((owner, materialization))
                if owner < materialization
                    && client_gui_status_response_observations[materialization]
                        .response_window_complete_before_observation =>
            {
                "owner_before_post_completion_materialization"
            }
            Some((owner, materialization)) if owner == materialization => "same_unit",
            Some((owner, materialization)) if owner < materialization => {
                "owner_before_materialization"
            }
            Some(_) => "materialization_before_owner",
            None if client_gui_status_response_observation_owner_units != 0
                && client_gui_status_response_observation_materialization_units == 0 =>
            {
                "owner_only"
            }
            ...
            None if client_gui_status_response_observation_authority_mismatch_only => {
                "authority_context_mismatch"
            }
            None if client_gui_status_response_observation_missing_pre_completion_owner_anchor => {
                "missing_pre_completion_owner_anchor"
            }
            None => "none",
        };
    let client_gui_status_response_observation_live_gui_units =
        client_gui_status_response_observations
            .iter()
            .filter(|observation| observation.live_gui_records != 0)
            .count();
    let client_gui_status_response_observation_live_gui_records =
        client_gui_status_response_observations
            .iter()
            .fold(0u64, |records, observation| {
                records.saturating_add(u64::from(observation.live_gui_records))
            });
    let client_gui_status_response_observation_live_gui_fragment_bits =
        client_gui_status_response_observations
            .iter()
            .fold(0u64, |bits, observation| {
                bits.saturating_add(u64::from(observation.live_gui_fragment_bits))
            });
    let current_player_status_binding = bridge.current_player_status_binding;
    let current_player_known_visible_equipment =
        current_player_known_visible_equipment(bridge, protocol, authorization_context);
    ...
    let current_player_known_visible_equipment_slot_count = current_player_known_visible_equipment
        ...
        .unwrap_or(0);
    // Prefer the exact server Inventory claim when it is also present in the
    // Status-authorized P/5 view. When HG's latest Inventory claim names a
    // different item, select a deterministic currently-proven P/5 member for
    // one initial false-branch probe. This does not infer an Inventory slot:
    // P/5 visible selectors and Inventory response slots are distinct
    // decompile-backed namespaces.
    let confirmed_claim_visible_equipment_match = last_known
        .then(|| {
            current_player_known_visible_equipment
                .as_ref()
        ...
        .flatten();
    let status_authorized_visible_equipment_candidate = current_player_known_visible_equipment
        .as_ref()
        .ok()
        .and_then(|view| {
            view.first_exact_proven_item(object_registry)
    ...
    // A cached server claim remains the richer source only when its complete
    // proof chain is currently usable. An old, malformed, undispatched, or
    // tombstoned claim must not suppress the independent Status/P/5 source.
    let confirmed_claim_authorization_response = last_known
        .then(|| {
    ...
    let confirmed_claim_is_eligible = last_known
        ...
        && current_player_status_binding
            .is_some_and(|binding| binding.queued_update_index >= last.update_index)
        && confirmed_claim_visible_equipment_match.is_some()
        && (protocol.last_client_equip_toggle.is_none()
            || confirmed_claim_has_matching_client_action);
    let recommended_client_inventory_equip_toggle_source = if confirmed_claim_is_eligible
        || (last_known && status_authorized_visible_equipment_candidate.is_none())
    {
        InventoryEquipToggleRecommendationSource::ConfirmedServerInventoryEquipClaim
    } else {
        InventoryEquipToggleRecommendationSource::StatusAuthorizedCurrentPlayerVisibleEquipment
    };
    let recommendation_uses_confirmed_server_claim = matches!(
        recommended_client_inventory_equip_toggle_source,
        InventoryEquipToggleRecommendationSource::ConfirmedServerInventoryEquipClaim
    );
    let recommended_client_inventory_equip_toggle_candidate =
        if recommendation_uses_confirmed_server_claim {
            last_known.then_some(InventoryEquipToggleRecommendationCandidate {
                object_id: last.object_id,
                update_index: last.update_index,
                server_inventory_minor: last.minor,
                alternate_inventory_context: last.alternate_inventory_context,
                server_inventory_equip_slot: last.equip_slot,
            })
        } else {
            status_authorized_visible_equipment_candidate.map(|(_, _, object_id)| {
                InventoryEquipToggleRecommendationCandidate {
                    object_id,
                    update_index: current_player_status_binding
                        .map(|binding| binding.queued_update_index)
                        .unwrap_or(0),
                    server_inventory_minor: 0,
                    alternate_inventory_context: false,
                    server_inventory_equip_slot: 0,
                }
            })
        };
    let recommended_client_inventory_equip_toggle_candidate_object_id =
        recommended_client_inventory_equip_toggle_candidate
            .map(|candidate| candidate.object_id)
            .unwrap_or(0);
    let recommended_client_inventory_equip_toggle_object_status =
        recommended_client_inventory_equip_toggle_candidate
            .map(|candidate| object_registry.inventory_item_object_status(candidate.object_id))
            .unwrap_or(semantic::InventoryItemObjectStatus::Unknown);
    let recommended_client_inventory_equip_toggle_authorization_response =
        recommendation_uses_confirmed_server_claim
            .then_some(confirmed_claim_authorization_response)
            .flatten();
    let recommended_client_inventory_equip_toggle_visible_equipment_match =
        current_player_known_visible_equipment
            ...
            .ok()
            .and_then(|view| {
                view.exact_visible_slot_for_object(
                    recommended_client_inventory_equip_toggle_candidate_object_id,
                )
                .map(|visible_slot| {
                    (
                        view.owner_object_id,
                        visible_slot,
                        recommended_client_inventory_equip_toggle_candidate_object_id,
                    )
                })
    ...
    let current_player_status_binding_covers_update =
        current_player_status_binding.is_some_and(|binding| {
            binding.queued_update_index
                >= recommended_client_inventory_equip_toggle_candidate
                    .map(|candidate| candidate.update_index)
                    .unwrap_or(u64::MAX)
        });
    ...
    let recommended_client_inventory_equip_toggle_authorization_kind =
        if !recommendation_uses_confirmed_server_claim {
            if protocol.last_client_equip_toggle.is_none() {
                "initial_status_authorized_visible_equipment"
            } else {
                "status_authorized_visible_equipment_probe_consumed"
            }
        } else if protocol.last_client_equip_toggle.is_none() {
    ...
    let recommended_client_inventory_equip_toggle_blocked_reason =
        if recommendation_uses_confirmed_server_claim {
            if !last_known {
        ...
        } else if current_player_known_visible_equipment.is_err() {
            match current_player_known_visible_equipment
                .as_ref()
                .expect_err("checked unavailable Status-authorized P/5 view")
            {
                CurrentPlayerKnownVisibleEquipmentBlockedReason::MissingStatusBinding => {
                    "current_player_inventory_status_unconfirmed"
                }
                CurrentPlayerKnownVisibleEquipmentBlockedReason::AreaMismatch => {
                    "current_player_inventory_status_stale"
                }
                CurrentPlayerKnownVisibleEquipmentBlockedReason::ControlEpochMismatch
                | CurrentPlayerKnownVisibleEquipmentBlockedReason::OwnerMismatch => {
                    "current_player_inventory_status_owner_mismatch"
                }
            }
        } else if status_authorized_visible_equipment_candidate.is_none() {
            "current_player_visible_equipment_positive_match_unavailable"
        } else if protocol.last_client_equip_toggle.is_some() {
            "status_authorized_visible_equipment_probe_consumed"
    ...
    let recommended_client_inventory_equip_toggle_output =
        (recommended_client_inventory_equip_toggle_blocked_reason == "none")
            .then_some(recommended_client_inventory_equip_toggle_candidate)
            .flatten();
    ...
    let recommended_client_inventory_equip_toggle_minor =
        recommended_client_inventory_equip_toggle_output
            .map(|output| output.server_inventory_minor)
            .unwrap_or(0);
    ...
    let recommended_client_inventory_equip_toggle_equip_slot =
        recommended_client_inventory_equip_toggle_output
            .map(|output| output.server_inventory_equip_slot)
            .unwrap_or(0);
    ...
    let fields = format!(
        ...
        recommended_client_inventory_equip_toggle_source.as_str(),
        recommended_client_inventory_equip_toggle_authorization_kind,
        recommended_client_inventory_equip_toggle_authorization_action_epoch,
        recommended_client_inventory_equip_toggle_authorization_response_ordinal,
        recommended_client_inventory_equip_toggle_authorization_response_operation,
    ...
    let fields = format!(
        concat!(
            "{}",
            ",\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observations_observed\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observations_retained\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observations_evicted\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_authority_contexts\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_first_queued_update_index\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_first_server_sequence\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_first_server_peer_ack_sequence\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_first_ack_sequence\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_first_area_client_area_packets\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_first_control_epoch\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_first_current_player_object_id_known\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_first_current_player_object_id\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_first_current_player_object_id_hex\": \"0x{:08X}\",\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_last_queued_update_index\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_last_server_sequence\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_last_server_peer_ack_sequence\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_last_ack_sequence\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_last_area_client_area_packets\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_last_control_epoch\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_last_current_player_object_id_known\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_last_current_player_object_id\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_last_current_player_object_id_hex\": \"0x{:08X}\",\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_forwarded_units\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_units_after_response_completion\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_inventory_records\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_owner_units\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_pre_completion_owner_units\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_owner_records\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_owner_mask_union\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_owner_mask_union_hex\": \"0x{:04X}\",\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_materialization_units\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_materialized_item_object_ids\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_queued_candidate_units\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_same_unit_owner_materialization_units\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_owner_only_units\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_materialization_only_units\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_cross_unit_owner_materialization\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_authority_correlated_owner_materialization\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_correlated_pair_includes_post_completion_unit\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_authority_mismatch_only\": {},\n",
            "  \"inventory_equipment_bridge_output_client_gui_status_response_observation_missing_pre_completion_owner_anchor\": {},\n",
        ...
        client_gui_status_response_observations_retained,
        bridge.client_gui_status_response_observations_evicted,
        client_gui_status_response_observation_authority_contexts,
        first_client_gui_status_response_observation.queued_update_index,
        first_client_gui_status_response_observation.server_sequence,
        first_client_gui_status_response_observation.server_peer_ack_sequence,
        first_client_gui_status_response_observation.ack_sequence,
        first_client_gui_status_response_observation.area_client_area_packets,
        first_client_gui_status_response_observation.control_epoch,
        first_client_gui_status_response_observation
        ...
        first_client_gui_status_response_observation
            .current_player_object_id
            .unwrap_or(0),
        last_client_gui_status_response_observation.queued_update_index,
        last_client_gui_status_response_observation.server_sequence,
        last_client_gui_status_response_observation.server_peer_ack_sequence,
        last_client_gui_status_response_observation.ack_sequence,
        last_client_gui_status_response_observation.area_client_area_packets,
        last_client_gui_status_response_observation.control_epoch,
        last_client_gui_status_response_observation
            .current_player_object_id
            .is_some(),
        last_client_gui_status_response_observation
            .current_player_object_id
        ...
        last_client_gui_status_response_observation
            .current_player_object_id
            .unwrap_or(0),
        client_gui_status_response_observation_forwarded_units,
        client_gui_status_response_observation_units_after_response_completion,
        client_gui_status_response_observation_inventory_records,
        client_gui_status_response_observation_owner_units,
        client_gui_status_response_observation_pre_completion_owner_units,
        client_gui_status_response_observation_owner_records,
        client_gui_status_response_observation_owner_mask_union,
        client_gui_status_response_observation_owner_mask_union,
        client_gui_status_response_observation_materialization_units,
        client_gui_status_response_observation_materialized_item_object_ids,
        client_gui_status_response_observation_queued_candidate_units,
        client_gui_status_response_observation_same_unit_owner_materialization_units,
        client_gui_status_response_observation_owner_only_units,
        client_gui_status_response_observation_materialization_only_units,
        client_gui_status_response_observation_cross_unit_owner_materialization,
        client_gui_status_response_observation_authority_correlated_owner_materialization,
        client_gui_status_response_observation_correlated_pair_includes_post_completion_unit,
        client_gui_status_response_observation_authority_mismatch_only,
        client_gui_status_response_observation_missing_pre_completion_owner_anchor,
        client_gui_status_response_observation_owner_materialization_chronology,
    ...
    let staged_status_open_payload_hex = bridge
        ...
        .unwrap_or_default();
    let staged_status_native_cancel_requested =
        bridge.staged_client_gui_status_native_cancel_requested();
    let staged_status_native_cancel_payload_hex = staged_status_native_cancel_requested
        .then(|| hex_encode_upper(&client_gui_inventory::build_ee_self_status_payload(true)))
        .unwrap_or_default();
    let fields = format!(
        concat!(
            "{}",
            ...
            "  \"inventory_equipment_bridge_staged_status_in_flight_native_cancel_enabled\": {},\n",
            "  \"inventory_equipment_bridge_staged_status_in_flight_native_cancel_requested\": {},\n",
            ...
            "  \"recommended_client_gui_inventory_status_native_cancel_payload_available\": {},\n",
            "  \"recommended_client_gui_inventory_status_native_cancel_payload_kind\": \"GuiInventory_Status\",\n",
            "  \"recommended_client_gui_inventory_status_native_cancel_payload_hex\": \"{}\",\n",
            "  \"recommended_client_gui_inventory_status_native_cancel_object_id\": 4294967293,\n",
            "  \"recommended_client_gui_inventory_status_native_cancel_object_id_hex\": \"0xFFFFFFFD\",\n",
            "  \"recommended_client_gui_inventory_status_native_cancel_player_inventory_gui\": true,\n",
            ...
            "  \"inventory_equipment_bridge_staged_status_last_committed_close_sequence\": {},\n",
            "  \"inventory_equipment_bridge_staged_status_last_committed_close_generation\": {},\n",
        ...
        bridge.staged_client_gui_status_in_flight_native_cancel_enabled,
        staged_status_native_cancel_requested,
        bridge
        ...
        staged_status_native_cancel_requested,
        staged_status_native_cancel_payload_hex,
        bridge
        ...
        bridge
            ...
            .unwrap_or_default(),
        bridge
            .staged_client_gui_status_last_committed_close_sequence
            .unwrap_or_default(),
        bridge
            .staged_client_gui_status_last_committed_close_generation
            .unwrap_or_default(),
        bridge
...
mod tests {
    ...
    fn commit_server_receive_slot(
        state: &mut server_replay::ServerReliableSlotState,
        packet: &[u8],
    ) -> server_replay::ServerReliableSlotKey {
        let view = MFrameView::parse(packet).expect("committed server reliable frame");
        let prepared =
            server_replay::prepare_source_slot(state, packet, &view).expect("pin server source");
        let admission =
            server_replay::prepare_source_dispatch(state, prepared).expect("admit server source");
        let server_replay::ServerReliableDispatchAdmission::Ready(key) = admission else {
            panic!("contiguous server source should be ready: {admission:?}");
        };
        server_replay::stage_source_dispatch(state, key).expect("stage server source");
        assert_eq!(
            server_replay::finish_source_dispatch(state, true),
            Some(key)
        );
        key
    }

    fn commit_client_receive_slot(
        state: &mut client_replay::ClientReliableReplayState,
        sequence: u16,
    ) -> client_replay::ClientReliableSlotKey {
        let slot = state
            .slots
            .iter()
            .find(|slot| slot.key.sequence == sequence)
            .expect("client source slot should be pinned");
        let key = slot.key;
        if slot.replay.is_none() {
            let packet = slot.packet.clone();
            client_replay::stage_translation(state, key, VerifiedFamily::ClientInput, Some(packet))
                .expect("stage strict client disposition");
        }
        assert_eq!(
            client_replay::release_committed_prefix(state, key),
            Some(key)
        );
        key
    }
    ...
    #[test]
    fn committed_receive_frontier_admits_next_source_despite_output_retention() {
        ...
        for sequence in 12..28 {
            ...
            assert_eq!(
                commit_server_receive_slot(&mut state, &packet).sequence,
                sequence
            );
        }

        assert_eq!(state.receive_start, Some(28));
        assert!(state.slots.is_empty());
        assert_eq!(
            state.output_sources.len(),
        ...
        let server_replay::PreparedServerReliableSource::Pinned(key) = prepared else {
            panic!("source at the advanced receive frontier should be admitted: {prepared:?}");
        };
        assert!(matches!(
            server_replay::prepare_source_dispatch(&mut state, prepared)
                .expect("admit source at advanced frontier"),
            server_replay::ServerReliableDispatchAdmission::Ready(ready) if ready == key
        ));
        assert_eq!(state.slots.len(), 1);
        assert_eq!(state.output_sources.len(), 16);
    }
    ...
    fn strict_session_translator_for_test() -> crate::translate::SessionTranslator {
        let template = crate::translate::Translator {
            ...
            synthetic_status_in_flight_native_cancel_experiment: false,
    ...
    fn direct_ordered_successor_effects_are_atomic_at_final_validation() {
        ...
        assert!(
            state
                ...
                .pending_client_to_server_packets
                .iter()
                .any(|pending| pending.reason == local_ack::COMMITTED_SERVER_RECEIVE_ACK_REASON)
        );
        assert_eq!(state.sequence.client_sequence_shifts.len(), 1);
        assert_eq!(
            state.login_waypoint.last_server_get_waypoint_sequence,
    ...
    fn direct_loadbar_rejection_retains_source_slot_and_retries_from_clean_effects() {
        ...
        assert_eq!(state.server_reliable_slots.output_sources.len(), 1);
    }
    ...
    fn consumed_reassembly_frontier_waits_for_ack_carrier_effect_validation() {
        ...
        let mut state = SessionState::default();
        state.synthetic_area.synthesize_loadbar = false;
        queue_proxy_owned_ack_for_committed_client_frontier(&mut state, 17)
        ...
        state.client_ack.pending.pending_committed_receive_ack = None;
        assert!(matches!(
            translate_server_to_client(&first, &mut state)
    ...
    fn retired_server_identity_classifies_exact_conflicting_and_unknown_stale_data() {
        ...
        assert_eq!(commit_server_receive_slot(&mut state, &source).sequence, 36);
        assert_eq!(state.receive_start, Some(37));
        assert_eq!(state.output_sources.len(), 1);
        assert_eq!(
    ...
    fn out_of_window_server_reliable_data_requeues_current_cumulative_ack() {
        ...
        let mut translator = strict_session_translator_for_test();
        assert_eq!(
            commit_server_receive_slot(&mut translator.m_state.server_reliable_slots, &source,)
                .sequence,
            36
        );
        assert_eq!(
    ...
    fn out_of_window_server_ack_waits_for_strict_carrier_acceptance() {
        ...
        let mut state = SessionState::default();
        assert_eq!(
            commit_server_receive_slot(&mut state.server_reliable_slots, &source).sequence,
            36
        );
        assert_eq!(
    ...
    #[test]
    fn server_receive_and_output_retirement_use_independent_ack_owners() {
        let payload = crate::translate::loadbar::start_payload(2);
        ...
        assert_eq!(state.server_reliable_slots.receive_start, Some(36));
        assert!(state.server_reliable_slots.slots.is_empty());
        assert!(state.server_reliable_slots.output_sources.is_empty());
        assert_eq!(
            state.sequence.server_sequence_epoch_source_floor,
            Some(server_replay::ServerReliableSlotKey {
                sequence: 36,
    ...
    fn expanded_server_output_retires_only_after_terminal_ee_ack_is_accepted() {
        ...
        finish_server_to_client_emit_validation(&mut state, true);
        let source = state.server_reliable_slots.output_sources[0].key;
        seed_authoritative_server_insertion(
            &mut state,
        ...
        stage_direct_client_send_window(&mut state, &partial_emit)
            .expect("stage partial client output ownership");
        finish_client_to_server_emit_validation(&mut state, true);
        ...
        stage_direct_client_send_window(&mut state, &rejected_terminal_emit)
            .expect("stage rejected terminal client output ownership");
        finish_client_to_server_emit_validation(&mut state, false);
        assert_eq!(state.server_reliable_slots.output_sources.len(), 1);
        assert_eq!(state.sequence.server_output_ack_spans.len(), 1);
        ...
        stage_direct_client_send_window(&mut state, &accepted_terminal_emit)
            .expect("stage accepted terminal client output ownership");
        finish_client_to_server_emit_validation(&mut state, true);
        assert_eq!(state.server_reliable_slots.receive_start, Some(62));
        assert!(state.server_reliable_slots.slots.is_empty());
        assert!(state.server_reliable_slots.output_sources.is_empty());
    ...
    fn consumed_client_output_does_not_deliver_its_source_ack() {
        ...
        let mut state = SessionState::default();
        assert_eq!(
            commit_server_receive_slot(&mut state.server_reliable_slots, &server_source).sequence,
            35
        );
    ...
    #[test]
    fn dropped_server_payload_carrier_does_not_retire_client_receive_sources() {
        let mut state = SessionState::default();
        for sequence in 7..=9 {
    ...
    #[test]
    fn exact_server_ack_output_cannot_retire_client_receive_sources() {
        let mut state = SessionState::default();
        for sequence in 7..=9 {
            let packet = client_reliable_m_frame(sequence, 30, &[0x70, 0x0D, 0x01, 0]);
    ...
    fn staged_status_native_cancel_state_before_close_drain() -> SessionState {
        ...
        inventory_equipment::maybe_queue_inventory_equipment_bridge_output(&mut state, 20, 7)
            .expect("stage close for native Status cancellation test");
        state
    }

    #[test]
    fn rejected_staged_close_drain_never_arms_in_flight_native_cancel() {
        let mut state = staged_status_native_cancel_state_before_close_drain();
        ...
        stage_pending_client_send_window(&mut state, &close)
            .expect("stage rejected close in speculative Diamond send window");

        finish_pending_client_drain_emit_validation(&mut state, false);

        assert!(state.diamond_client_send_window.slots.is_empty());
        assert_eq!(state.sequence.pending_client_to_server_packets.len(), 1);
        assert_eq!(
            state.inventory_equipment.staged_client_gui_status_state,
            state::InventoryEquipmentStagedClientGuiStatusState::CloseQueued
        );
        assert_eq!(
            state
                .inventory_equipment
        ...
        assert!(body.contains(
            "\"inventory_equipment_bridge_staged_status_in_flight_native_cancel_requested\": false"
        ));
        assert!(body.contains(
            "\"recommended_client_gui_inventory_status_native_cancel_payload_available\": false"
        ));
        assert!(body.contains(
            "\"recommended_client_gui_inventory_status_native_cancel_payload_hex\": \"\""
        ));
    ...
    fn native_status_after_staged_close_commit_keeps_shift_and_maps_after_close() {
        ...
        assert_eq!(
            state.inventory_equipment.staged_client_gui_status_state,
            state::InventoryEquipmentStagedClientGuiStatusState::CloseInFlight
        );
        assert_eq!(
            state
                .inventory_equipment
    ...
    #[test]
    fn native_status_cancellation_of_in_flight_close_survives_late_raw_ack() {
        let mut state = staged_status_native_cancel_state_before_close_drain();
        let close = take_pending_client_to_server_packets(&mut state).expect("drain staged close");
        stage_pending_client_send_window(&mut state, &close)
        ...
        let native = client_reliable_m_frame(
            11,
            21,
            &client_gui_inventory::build_ee_self_status_payload(true),
        );
        begin_client_to_server_emit_validation(&mut state, &native)
            .expect("native Status validation transaction");
        let translated = translate_client_to_server(&native, &mut state)
            .expect("native Status should cancel the held synthetic open");
        let translated_packet = match &translated {
            Emit::VerifiedPackets { packets, .. } => packets.first().expect("native Status output"),
            other => panic!("expected one verified native Status, got {other:?}"),
        };
        ...
        observe_validated_server_source_ack(&mut state, 11);
        ...
        assert_eq!(
            state
                ...
                .slots
                .front()
                .expect("native Status remains in flight")
                .key
                .sequence,
            12
        );
        assert_eq!(
            state.inventory_equipment.staged_client_gui_status_state,
            state::InventoryEquipmentStagedClientGuiStatusState::Cancelled,
            "the close's later raw ACK must not resurrect the held open"
        );
        assert_eq!(
            state.sequence.client_sequence_shifts,
            vec![SequenceShift { base: 11, delta: 1 }],
            "retiring the cancelled close must not discard its owned insertion shift"
        );
        assert_eq!(
            state
                .inventory_equipment
        ...
        assert!(!inventory_equipment::staged_client_gui_status_open_ready(
            &state
        ));
        assert!(!pending_client_drain_has_work(&state));
        assert!(matches!(
            take_pending_client_to_server_packets(&mut state)
                .expect("late ACK should leave no synthetic open work"),
            Emit::Consumed
        ));
    }
    ...
    fn acknowledged_client_replay_collapses_without_semantic_effects() {
        ...
        assert_eq!(state.semantic.ui.inventory_packets, 1);
        assert_eq!(
            commit_client_receive_slot(&mut state.client_reliable_replays, 78).sequence,
            78
        );
    ...
    fn client_emit_effects_rollback_but_valid_source_transport_survives_rejection() {
        ...
        stage_direct_client_send_window(&mut state, &accepted)
            .expect("accepted retry should stage Diamond output ownership");
        finish_client_to_server_emit_validation(&mut state, true);
        assert_eq!(state.sequence.latest_client_ack_from_client, Some(32));
        assert_eq!(state.semantic.ui.inventory_packets, 1);
        assert_eq!(
    ...
    fn client_type_zero_sequence_zero_wraps_generation_and_replays_retransmit() {
        ...
        state.client_reliable_replays.receive_start = Some(0);
        state.client_reliable_replays.origin_generation = 1;
        state
            .sequence
            .client_sequence_shifts
    ...
    #[test]
    fn delayed_pre_wrap_client_slot_replays_while_post_wrap_waits() {
        let mut state = SessionState::default();
        ...
        assert_eq!(state.client_reliable_replays.origin_generation, 0);
        assert_eq!(state.client_reliable_replays.receive_start, Some(u16::MAX));
        assert_eq!(state.semantic.ui.inventory_packets, 1);
        assert_eq!(state.client_reliable_replays.exact_replays, 1);
    ...
    fn reliable_client_slot_window_rejects_outside_without_evicting_live_slots() {
        ...
        for sequence in 7..=14 {
            assert_eq!(
                commit_client_receive_slot(&mut state.client_reliable_replays, sequence).sequence,
                sequence
            );
        }
    ...
    fn out_of_window_client_reliable_data_requeues_current_cumulative_ack() {
        ...
        assert_eq!(
            commit_client_receive_slot(&mut translator.m_state.client_reliable_replays, 7).sequence,
            7
        );
    ...
    fn retired_client_identity_history_is_bounded_to_receive_window() {
        ...
        for sequence in 0..=client_replay::MAX_CLIENT_RELIABLE_SLOTS as u16 {
            ...
            assert_eq!(
                commit_client_receive_slot(&mut state, sequence).sequence,
                sequence
            );
            packets.push(packet);
        }
    ...
    #[test]
    fn client_dispatch_and_release_wait_for_contiguous_strict_frontier() {
        let payload = &[0x70, 0x0D, 0x01, 0];
        let mut client_state = client_replay::ClientReliableReplayState::default();
        let client_7 = client_reliable_m_frame(7, 30, payload);
        let client_7_view = MFrameView::parse(&client_7).expect("frontier client source");
        let prepared_7 =
            client_replay::prepare_source_slot(&mut client_state, &client_7, &client_7_view)
                .expect("pin frontier client source");
        assert!(matches!(
            client_replay::prepare_source_dispatch(&mut client_state, &prepared_7)
                .expect("admit frontier client source"),
            client_replay::ClientReliableDispatchAdmission::Ready(_)
        ));

        let client_9 = client_reliable_m_frame(9, 30, payload);
        let client_9_view = MFrameView::parse(&client_9).expect("future client source");
        let prepared_9 =
            client_replay::prepare_source_slot(&mut client_state, &client_9, &client_9_view)
                .expect("pin future client source");
        assert!(matches!(
            client_replay::prepare_source_dispatch(&mut client_state, &prepared_9)
                .expect("buffer future client source"),
        ...
        assert!(matches!(
            client_replay::prepare_source_dispatch(&mut client_state, &prepared_gap)
                .expect("admit filled client gap"),
            client_replay::ClientReliableDispatchAdmission::Ready(_)
        ));
        ...
        let server_40 = client_reliable_m_frame(40, 30, payload);
        assert_eq!(
            commit_server_receive_slot(&mut server_state, &server_40).sequence,
            40
        );
        let server_42 = client_reliable_m_frame(42, 30, payload);
        let server_42_view = MFrameView::parse(&server_42).expect("sparse server source");
        let prepared_42 =
            server_replay::prepare_source_slot(&mut server_state, &server_42, &server_42_view)
                .expect("pin sparse server source");
        assert!(matches!(
            server_replay::prepare_source_dispatch(&mut server_state, prepared_42)
                .expect("defer sparse server source"),
            server_replay::ServerReliableDispatchAdmission::Future { .. }
        ));
        assert_eq!(
        ...
        assert_eq!(server_state.receive_start, Some(41));
        assert_eq!(server_state.slots.len(), 1);
        assert_eq!(server_state.output_sources.len(), 1);
        let server_gap = client_reliable_m_frame(41, 30, payload);
        assert_eq!(
            commit_server_receive_slot(&mut server_state, &server_gap).sequence,
            41
        );
        let matched_42 =
            server_replay::prepare_source_slot(&mut server_state, &server_42, &server_42_view)
                .expect("match deferred server source");
        let server_replay::ServerReliableDispatchAdmission::Ready(key_42) =
            server_replay::prepare_source_dispatch(&mut server_state, matched_42)
                .expect("admit contiguous deferred server source")
        else {
            panic!("server source 42 should become ready after source 41");
        };
        server_replay::stage_source_dispatch(&mut server_state, key_42).expect("stage source 42");
        assert_eq!(
            server_replay::finish_source_dispatch(&mut server_state, true),
            Some(key_42)
        );
        assert_eq!(
            server_replay::retire_through_client_ack(&mut server_state, 42).len(),
        ...
        assert!(server_state.output_sources.is_empty());
    }

    #[test]
    fn session_translator_releases_client_prefix_after_diamond_output_commit() {
        let mut translator = strict_session_translator_for_test();
        translator.m_state.client_reliable_replays.receive_start = Some(7);
        let first_payload = client_gui_inventory::build_status_payload(
            client_gui_inventory::DIAMOND_CURRENT_PLAYER_OBJECT_ID,
            true,
        );
        let future_payload = client_gui_inventory::build_status_payload(
            client_gui_inventory::DIAMOND_CURRENT_PLAYER_OBJECT_ID,
            false,
        );
        let future = client_reliable_m_frame(8, 0, &future_payload);
        let future_emit =
            translator.translate(crate::translate::Direction::ClientToServer, &future);
        assert!(!matches!(future_emit, Emit::Drop));
        assert_eq!(translator.m_state.semantic.ui.inventory_packets, 0);
        assert_eq!(
        ...
        let first = client_reliable_m_frame(7, 0, &first_payload);
        let first_emit = translator.translate(crate::translate::Direction::ClientToServer, &first);
        assert!(!matches!(first_emit, Emit::Drop | Emit::Consumed));
        assert_eq!(translator.m_state.semantic.ui.inventory_packets, 1);
        assert_eq!(
            translator.m_state.client_reliable_replays.receive_start,
            Some(8)
        );
        assert_eq!(translator.m_state.client_reliable_replays.slots.len(), 1);
        assert_eq!(translator.m_state.diamond_client_send_window.slots.len(), 1);

        let deferred = translator
            .take_deferred_client_to_server_emit()
            .expect("strict commit should expose the retained successor");
        assert!(!matches!(deferred, Emit::Drop | Emit::Consumed));
        assert_eq!(translator.m_state.semantic.ui.inventory_packets, 2);
        assert_eq!(
            translator.m_state.client_reliable_replays.receive_start,
            Some(9)
        );
        assert!(translator.m_state.client_reliable_replays.slots.is_empty());
        assert_eq!(translator.m_state.diamond_client_send_window.slots.len(), 2);
        assert_eq!(
            translator
                .m_state
                .client_ack
    ...
    fn quickbar_hint_augmentation_serializes_confirmed_inventory_replay() {
        ...
        assert!(
            body.contains("\"recommended_client_inventory_equip_toggle_payload_available\": true")
        );
        assert!(body.contains(
            "\"recommended_client_inventory_equip_toggle_source\": \"status_authorized_current_player_visible_equipment\""
        ));
        assert!(body.contains(
            "\"recommended_client_inventory_equip_toggle_authorization_kind\": \"initial_status_authorized_visible_equipment\""
        ));
        ...
        assert!(
            stale_update_body
                .contains("\"recommended_client_inventory_equip_toggle_update_index\": 13")
        );
    ...
    fn inventory_equip_toggle_initial_probe_uses_lowest_proven_status_authorized_p5_member() {
        ...
        protocol
            .visible_equipment_slots_by_owner
            .insert((OWNER, 1), UNPROVEN_ITEM);
        protocol
            .visible_equipment_slots_by_owner
            .insert((OWNER, 2), ITEM_A);
        protocol
            .visible_equipment_slots_by_owner
            .insert((OWNER, 0x10), ITEM_B);
        let mut object_registry = semantic::ObjectRegistry::default();
        object_registry.observe_materialized_item_object_ids(&[ITEM_A, ITEM_B]);
        let authority = InventoryEquipToggleAuthorizationContext {
        ...
        assert!(initial.contains(
            "\"recommended_client_inventory_equip_toggle_payload_hex\": \"700C0B0B0000004400008080\""
        ));
        assert!(initial.contains(
            "\"recommended_client_inventory_equip_toggle_object_id_hex\": \"0x80000044\""
        ));
        assert!(initial.contains("\"recommended_client_inventory_equip_toggle_update_index\": 7"));
        assert!(
            initial.contains(
                "\"recommended_client_inventory_equip_toggle_server_inventory_minor\": 0"
            )
        );
        assert!(initial.contains(
            "\"recommended_client_inventory_equip_toggle_server_inventory_equip_slot\": 0"
        ));
        assert!(initial.contains(
        ...
        let stale = augment_quickbar_item_refresh_hint_with_bridge_output_and_protocol_state(
            "{\n  \"kind\": \"quickbar_item_refresh_idle\"\n}\n".to_string(),
            &bridge,
            &object_registry,
            &mut semantic::InventoryEquipmentProtocolState::default(),
            stale_area,
        );
        ...
        let fresh_area_authority = InventoryEquipToggleAuthorizationContext {
            current_controlled_object_id: Some(OWNER),
            current_control_epoch: 2,
            area_client_area_packets: 2,
        };
        let fresh_area = augment_quickbar_item_refresh_hint_with_bridge_output_and_protocol_state(
            "{\n  \"kind\": \"quickbar_item_refresh_idle\"\n}\n".to_string(),
            &bridge,
            &object_registry,
            &mut protocol,
    ...
    fn inventory_equip_toggle_rearm_requires_fresh_matching_response_epoch() {
        ...
        assert!(
            stale_status_update
                .contains("\"recommended_client_inventory_equip_toggle_update_index\": 11")
        );
        bridge
            .current_player_status_binding
            .as_mut()
    ...
    #[test]
    fn quickbar_hint_withholds_native_status_before_close_commit() {
        let state = staged_status_native_cancel_state_before_close_drain();
        assert_eq!(
            state.inventory_equipment.staged_client_gui_status_state,
            state::InventoryEquipmentStagedClientGuiStatusState::CloseQueued
        );
        ...
        assert!(body.contains(
            "\"inventory_equipment_bridge_staged_status_close_ack_open_state\": \"close_queued\""
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_staged_status_in_flight_native_cancel_requested\": false"
        ));
    ...
    #[test]
    fn quickbar_hint_requests_exact_native_status_only_for_committed_in_flight_close() {
        let claim = semantic::InventoryEquipmentClientGuiInventoryClaim {
            kind: semantic::InventoryEquipmentClientGuiInventoryClaimKind::Status,
            object_id: Some(client_gui_inventory::DIAMOND_CURRENT_PLAYER_OBJECT_ID),
            panel: None,
            player_inventory_gui: Some(true),
            rewritten_self_object_id: true,
        };
        let update = semantic::InventoryEquipmentBridgeStateUpdate {
            update_index: 7,
            emission_index: 4,
            consumer: semantic::InventoryEquipmentHandoffConsumer::ClientGuiInventory,
            event_index: 9,
            candidate: semantic::InventoryItemContextCandidate {
                object_id: 0x8001_5B01,
                proof: semantic::InventoryItemObjectProof::ActiveObject,
                source: semantic::InventoryItemContextCandidateSource::DirectOnly,
            },
            ready_objects: 1,
            deferred_feature25_only_objects: 0,
            server_inventory_claim: None,
        ...
        let mut bridge = state::InventoryEquipmentBridgeState::default();
        bridge.staged_client_gui_status_close_ack_open_enabled = true;
        bridge.staged_client_gui_status_in_flight_native_cancel_enabled = true;
        bridge.staged_client_gui_status_state =
            state::InventoryEquipmentStagedClientGuiStatusState::CloseInFlight;
        bridge.staged_client_gui_status_close_outputs = 1;
        bridge.staged_client_gui_status_close_commits = 1;
        bridge.staged_client_gui_status_last_committed_close_sequence = Some(11);
        bridge.staged_client_gui_status_last_committed_close_generation = Some(3);
        bridge.pending_staged_client_gui_status_open =
            Some(state::InventoryEquipmentPendingStagedClientGuiStatusOpen {
                update,
                open_claim: claim,
                current_player_object_id: 0x8000_5678,
                area_client_area_packets: 1,
                control_epoch: 2,
                close_trigger_client_sequence: 11,
                close_synthetic_sequence: 11,
                close_ack_sequence: 4,
                close_transport_identity: vec![1, 2, 3],
        ...
        assert!(body.contains(
            "\"inventory_equipment_bridge_staged_status_close_ack_open_state\": \"close_in_flight\""
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_staged_status_in_flight_native_cancel_requested\": true"
        ));
        assert!(body.contains("\"inventory_equipment_bridge_staged_status_close_commits\": 1"));
        assert!(body.contains(
            "\"inventory_equipment_bridge_staged_status_last_committed_close_sequence\": 11"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_staged_status_last_committed_close_generation\": 3"
        ));
        assert!(body.contains(
            "\"recommended_client_gui_inventory_status_native_cancel_payload_available\": true"
        ));
        assert!(body.contains(
            "\"recommended_client_gui_inventory_status_native_cancel_payload_hex\": \"700D010B000000FDFFFFFF90\""
        ));
    }
    ...
    fn quickbar_hint_augmentation_serializes_queued_client_gui_status_output() {
        ...
        bridge.record_client_gui_status_response_observation(
            state::InventoryEquipmentBridgeClientGuiStatusResponseObservation {
                queued_update_index: 12,
                server_sequence: 47,
                server_peer_ack_sequence: 82,
                ack_sequence: 82,
                current_player_object_id: Some(0xFFFF_FFEF),
                area_client_area_packets: 4,
                control_epoch: 7,
                inventory_records: 1,
                current_player_inventory_records: 2,
                current_player_inventory_mask_union: 0x2000,
                ..Default::default()
            },
        );
        bridge.record_client_gui_status_response_observation(
            state::InventoryEquipmentBridgeClientGuiStatusResponseObservation {
                queued_update_index: 12,
                // A second high-level LiveObject unit in the same enclosing M
                // frame shares transport sequence/ACK identity.
                server_sequence: 47,
                server_peer_ack_sequence: 82,
                ack_sequence: 82,
                current_player_object_id: Some(0xFFFF_FFEF),
        ...
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observations_observed\": 2"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observations_retained\": 2"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observations_evicted\": 0"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_first_server_sequence\": 47"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_last_server_sequence\": 47"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_authority_contexts\": 1"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_first_area_client_area_packets\": 4"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_first_control_epoch\": 7"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_owner_units\": 1"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_pre_completion_owner_units\": 1"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_owner_records\": 2"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_owner_mask_union_hex\": \"0x2000\""
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_materialization_units\": 1"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_materialized_item_object_ids\": 51"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_queued_candidate_units\": 1"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_same_unit_owner_materialization_units\": 0"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_cross_unit_owner_materialization\": true"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_authority_correlated_owner_materialization\": true"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_units_after_response_completion\": 0"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_correlated_pair_includes_post_completion_unit\": false"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_authority_mismatch_only\": false"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_owner_materialization_chronology\": \"owner_before_materialization\""
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_live_gui_records\": 51"
        ));
        assert!(body.contains(
    ...
    #[test]
    fn status_observation_hint_does_not_correlate_across_control_epochs() {
        let mut bridge = state::InventoryEquipmentBridgeState::default();
        bridge.record_client_gui_status_response_observation(
            state::InventoryEquipmentBridgeClientGuiStatusResponseObservation {
                queued_update_index: 9,
                server_sequence: 40,
                server_peer_ack_sequence: 70,
                ack_sequence: 70,
                current_player_object_id: Some(0xFFFF_FFEF),
        ...
        bridge.record_client_gui_status_response_observation(
            state::InventoryEquipmentBridgeClientGuiStatusResponseObservation {
                queued_update_index: 9,
                // Same enclosing transport frame, but ObjControl changed in a
                // later wire-ordered unit before its semantic observation.
                server_sequence: 40,
                server_peer_ack_sequence: 70,
                ack_sequence: 70,
                current_player_object_id: Some(0xFFFF_FFEE),
                area_client_area_packets: 3,
                control_epoch: 12,
                live_gui_records: 1,
                materialized_item_object_ids: 1,
        ...
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_authority_contexts\": 2"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_cross_unit_owner_materialization\": false"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_authority_correlated_owner_materialization\": false"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_correlated_pair_includes_post_completion_unit\": false"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_authority_mismatch_only\": true"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_missing_pre_completion_owner_anchor\": false"
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_owner_materialization_chronology\": \"authority_context_mismatch\""
        ));
        assert!(body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_first_control_epoch\": 11"
        ));
        assert!(body.contains(
        ...
        let post_completion_body = augment_quickbar_item_refresh_hint_with_bridge_output(
            "{\n  \"kind\": \"quickbar_item_refresh_candidate\"\n}\n".to_string(),
            &post_completion_bridge,
            &semantic::ObjectRegistry::default(),
        );
        assert!(post_completion_body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_units_after_response_completion\": 1"
        ));
        assert!(post_completion_body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_correlated_pair_includes_post_completion_unit\": true"
        ));
        assert!(post_completion_body.contains(
        ...
        evicted_anchor_bridge.record_client_gui_status_response_observation(
            state::InventoryEquipmentBridgeClientGuiStatusResponseObservation {
                current_player_object_id: Some(0xFFFF_FFEF),
                area_client_area_packets: 3,
                control_epoch: 11,
        ...
        for _ in 0..state::CLIENT_GUI_STATUS_RESPONSE_OBSERVATION_CAPACITY {
            evicted_anchor_bridge.record_client_gui_status_response_observation(
                state::InventoryEquipmentBridgeClientGuiStatusResponseObservation {
                    current_player_object_id: Some(0xFFFF_FFEF),
                    area_client_area_packets: 3,
        ...
        let evicted_anchor_body = augment_quickbar_item_refresh_hint_with_bridge_output(
            "{\n  \"kind\": \"quickbar_item_refresh_candidate\"\n}\n".to_string(),
            &evicted_anchor_bridge,
            &semantic::ObjectRegistry::default(),
        );
        assert!(evicted_anchor_body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observations_evicted\": 1"
        ));
        assert!(evicted_anchor_body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_pre_completion_owner_units\": 0"
        ));
        assert!(evicted_anchor_body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_authority_correlated_owner_materialization\": false"
        ));
        assert!(evicted_anchor_body.contains(
            "\"inventory_equipment_bridge_output_client_gui_status_response_observation_missing_pre_completion_owner_anchor\": true"
        ));
        assert!(evicted_anchor_body.contains(
    ...
    fn proxy_owned_client_ack_coalesces_and_releases_from_session_drain() {
        ...
        queue_proxy_owned_ack_for_committed_client_frontier(&mut state, 40).expect("queue ACK");
        queue_proxy_owned_ack_for_committed_client_frontier(&mut state, 42).expect("coalesce ACK");
    ...
    fn pending_server_drain_reject_restores_ack_queue_semantics_and_window_state() {
        ...
        queue_proxy_owned_ack_for_committed_client_frontier(&mut translator.m_state, 42)
