...
mod diagnostic_tests {
    ...
    fn item_failure_source_window_neighbor_fit_reports_focus_row_start() {
        ...
        let neighbors = live_object_source_window_item_neighbor_cursor_claims(
            ...
            claims[1].bit_start,
            claims[0].bit_end,
            &LiveObjectRewriteBitLedger::new(),
        );
    ...
    fn rewrite_bit_ledger_classifies_unowned_emitted_cursor_gap() {
        ...
        assert_eq!(
            (
                exact_cursor.emitted_gap_bit_start,
                exact_cursor.emitted_gap_bit_end
            ),
            (CNW_FRAGMENT_HEADER_BITS + 25, CNW_FRAGMENT_HEADER_BITS + 25)
        );
        assert_eq!(exact_cursor.source_relation, "after-previous-source-end");
        assert_eq!(exact_cursor.source_gap_bits, 0);
        assert_eq!(
            (
                exact_cursor.source_gap_bit_start,
                exact_cursor.source_gap_bit_end
            ),
            (CNW_FRAGMENT_HEADER_BITS + 19, CNW_FRAGMENT_HEADER_BITS + 19)
        );
        assert_eq!(
        ...
        assert_eq!(
            (
                shifted_candidate.emitted_gap_bit_start,
                shifted_candidate.emitted_gap_bit_end
            ),
            (CNW_FRAGMENT_HEADER_BITS + 25, CNW_FRAGMENT_HEADER_BITS + 27)
        );
        assert_eq!(shifted_candidate.source_relation, "unowned-source-gap");
        assert_eq!(shifted_candidate.source_gap_bits, 2);
        assert_eq!(
            (
                shifted_candidate.source_gap_bit_start,
                shifted_candidate.source_gap_bit_end
            ),
            (CNW_FRAGMENT_HEADER_BITS + 19, CNW_FRAGMENT_HEADER_BITS + 21)
        );
    ...
    #[test]
    fn item_update_cursor_failure_kind_uses_ledger_gap_shape() {
        let mut live = vec![0; 16];
        live[0] = b'A';
        live[1] = ITEM_OBJECT_TYPE;
        ...
        let mut ledger = LiveObjectRewriteBitLedger::new();
        assert_eq!(
            ledger.item_update_cursor_failure_kind(CNW_FRAGMENT_HEADER_BITS),
        ...
        assert_eq!(
            ledger.item_update_cursor_failure_kind(CNW_FRAGMENT_HEADER_BITS + 2),
            LiveObjectUpdateRewriteFailureKind::ItemUpdateCursorBeforeLedgerRow
        );
        assert_eq!(
            ledger.item_update_cursor_failure_kind(CNW_FRAGMENT_HEADER_BITS + 6),
    ...
    fn exact_placeable_add_reconciles_unique_area_static_appearance_word() {
        ...
        claim_payload_if_verified(&payload).expect("pre-rewrite exact A/09 claim");
        ...
        let claim = claim_payload_if_verified(&payload).expect("post-rewrite exact A/09 claim");
        assert_eq!(
            claim.mentions[0].placeable_appearance,
            Some(LiveObjectPlaceableAppearance {
                appearance: 0x0022,
        ...
        assert_eq!(
            ...
            "appearance reconciliation must not disturb add-state bits"
        );
    }
    ...
    fn exact_placeable_update_rewrites_resref_appearance_to_unique_area_static_word() {
        ...
        let fragment_bits = vec![false; CNW_FRAGMENT_HEADER_BITS];
        let mut payload =
            live_object_payload_from_parts(&live, &fragment_bits).expect("exact resref U/09");
    ...
    fn exact_placeable_add_reconciles_unique_area_static_state_with_optional_object_id() {
        ...
        live.extend_from_slice(&0x0011u16.to_le_bytes());
        live.extend_from_slice(&0u16.to_le_bytes());
        live.extend_from_slice(&optional_target.to_le_bytes());
        live.extend_from_slice(&visual_transform::EE_OBJECT_VISUAL_TRANSFORM_IDENTITY_BYTES);
        ...
        fragment_bits.extend([
            false, // direct CExoString name branch.
            false, // reputation/visual selector.
            true,  // optional object id bytes are present before the visual map.
            false, // static/plot stays packet-authored.
            false, // useable conflicts with the unique module static row.
            true,  // trap-disarmable conflicts with the unique module static row.
            false, // lockable conflicts with the unique module static row.
        ...
        let mut payload =
            live_object_payload_from_parts(&live, &fragment_bits).expect("exact A/09 payload");
        claim_payload_if_verified(&payload).expect("pre-rewrite optional-object A/09 exact claim");
        ...
        let rewritten_bits =
            bits::decode_msb_valid_bits(&payload[claim.declared..], CNW_FRAGMENT_HEADER_BITS)
                .expect("rewritten exact A/09 fragment bits");
        assert!(
            rewritten_bits[CNW_FRAGMENT_HEADER_BITS + 2],
            "the optional-object guard bit must stay tied to the byte layout"
        );
        assert_eq!(
            &payload[claim.declared
                - visual_transform::EE_OBJECT_VISUAL_TRANSFORM_IDENTITY_BYTES_LEN
                - 4
                ..claim.declared - visual_transform::EE_OBJECT_VISUAL_TRANSFORM_IDENTITY_BYTES_LEN],
            &optional_target.to_le_bytes(),
            "optional object id bytes must remain packet-authored"
        );
    }
    ...
    #[test]
    fn exact_placeable_add_identity_blocked_fixed_fields_select_unique_static_match() {
        ...
        assert_eq!(
            ...
            "only the add with one matching static row should count as fixed-field evidence"
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            "the selected A/09 remains fixed-width and packet-authored"
        );
        assert_eq!(
            claim.mentions[1].placeable_appearance,
            Some(LiveObjectPlaceableAppearance {
            ...
            "the synthesized following U/09 carries the module TemplateResRef"
        );
        assert_eq!(
            claim.mentions[1].fragment_bit_start, claim.mentions[0].fragment_bit_end,
    ...
    #[test]
    fn exact_placeable_add_output_equivalent_duplicate_rows_rewrite_visible_fields() {
        let object_id = 0x8000_3609u32;
        ...
        fragment_bits.extend([
            ...
            false, // no optional object id bytes.
            false, // static/plot stays packet-authored.
            false, // useable disagrees with the shared module state.
            false, // trap disarmable already matches.
            false, // lockable disagrees with the shared module state.
            false, // locked already matches.
            false, // unknown 0x1AC sibling stays packet-authored.
            true,  // name-valid stays packet-authored.
            false, // EE-only light/visual guard before the transform map.
        ...
        let mut payload =
            live_object_payload_from_parts(&live, &fragment_bits).expect("exact custom A/09");
        let pre_claim =
            claim_payload_if_verified(&payload).expect("pre-rewrite exact duplicate A/09 claim");
        assert_eq!(
            pre_claim.mentions[0].placeable_appearance,
            Some(LiveObjectPlaceableAppearance {
                appearance: 0xFFFE,
        ...
        assert_eq!(
            pre_claim.mentions[0].placeable_state,
            Some(LiveObjectPlaceableState {
                useable: Some(false),
                trap_disarmable: Some(false),
                lockable: Some(false),
        ...
        .expect("duplicate rows with identical add output should rewrite");
        assert_eq!(summary.add_records_examined, 1);
        assert_eq!(summary.add_records_rewritten, 1);
        assert_eq!(summary.exact_placeable_add_unique_targets, 1);
        assert_eq!(summary.exact_placeable_add_identity_blocked, 0);
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_fixed_fields, 0,
            "the packet fields did not match the module rows before rewrite"
        );
        assert_eq!(
        ...
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_add_output_equivalence, 1,
            "selection is allowed only because every static candidate emits the same A/09 output"
        );
        assert_eq!(
            summary.exact_placeable_add_identity_blocked_fixed_field_matches,
            0
        ...
        let claim =
            claim_payload_if_verified(&payload).expect("post-rewrite exact duplicate A/09 claim");
        assert_eq!(claim.mentions.len(), 1);
        assert_eq!(
            claim.mentions[0].placeable_appearance,
            Some(LiveObjectPlaceableAppearance {
            ...
            "A/09 output-equivalence rewrites only the fixed WORD"
        );
        assert_eq!(
            claim.mentions[0].placeable_state,
            Some(LiveObjectPlaceableState {
            ...
            "state bits use the shared module-backed output"
        );
        assert_eq!(
            claim.mentions[0].fragment_bit_start,
            CNW_FRAGMENT_HEADER_BITS
        );
        assert_eq!(
            claim.mentions[0].fragment_bit_end,
            CNW_FRAGMENT_HEADER_BITS + EE_PLACEABLE_ADD_DIRECT_EMPTY_GUARD_BITS
        );
    }

    #[test]
    fn exact_placeable_add_identity_resolves_by_following_position_update() {
        let object_id = 0x8000_3610u32;
        let z_raw = encode_ee_position_z(0.0).expect("test z should encode");
        ...
        fragment_bits.extend([
            ...
            false, // no optional object id bytes.
            false, // static/plot stays packet-authored.
            false, // useable conflicts with the selected module row.
            false, // trap disarmable already matches.
            false, // lockable conflicts with the selected module row.
            false, // locked already matches.
            false, // unknown 0x1AC sibling stays packet-authored.
        ...
        fragment_bits.extend([(z_raw & 0b10) != 0, (z_raw & 0b01) != 0]);
        let mut payload = live_object_payload_from_parts(&live, &fragment_bits)
            .expect("exact A/09 followed by position U/09");
        ...
        .expect("following parser-owned position update should select one static row for A/09");
        assert_eq!(summary.add_records_examined, 1);
        assert_eq!(summary.update_records_examined, 1);
        assert_eq!(summary.add_records_rewritten, 1);
        assert_eq!(summary.update_records_rewritten, 0);
        ...
        assert_eq!(
            summary.exact_placeable_update_identity_resolved_by_position, 1,
            "the following U/09 still records its own position-based identity proof"
        );
        assert_eq!(summary.exact_placeable_add_appearance_rewritten, 1);
        assert_eq!(summary.exact_placeable_add_state_rewritten, 1);
        assert_eq!(summary.exact_placeable_update_position_rewritten, 0);
        ...
        let position = claim.mentions[1]
            .position
            .expect("following U/09 position should stay parser-owned");
    ...
    #[test]
    fn exact_placeable_add_identity_resolves_by_preceding_position_update() {
        let object_id = 0x8000_3615u32;
        let z_raw = encode_ee_position_z(0.0).expect("test z should encode");
        ...
        let mut payload = live_object_payload_from_parts(&live, &fragment_bits)
            .expect("exact U/09 position followed by A/09 payload");
        ...
        .expect("preceding parser-owned position update should select one static row for A/09");
        assert_eq!(summary.add_records_examined, 1);
        assert_eq!(summary.update_records_examined, 1);
        assert_eq!(summary.add_records_rewritten, 1);
        assert_eq!(summary.update_records_rewritten, 0);
        ...
        assert_eq!(
            summary.exact_placeable_update_identity_resolved_by_position, 1,
            "the preceding U/09 still records its own position-based identity proof"
        );
        assert_eq!(summary.exact_placeable_add_appearance_rewritten, 1);
        assert_eq!(summary.exact_placeable_add_state_rewritten, 1);
        assert_eq!(summary.exact_placeable_update_position_rewritten, 0);
        ...
        let claim =
            claim_payload_if_verified(&payload).expect("post-rewrite exact U/09 + A/09 claim");
        assert_eq!(claim.mentions.len(), 2);
        assert_eq!(claim.mentions[1].record_offset, add_offset);
        let position = claim.mentions[0]
            .position
            .expect("preceding U/09 position should stay parser-owned");
        assert_eq!(position.x_raw, 1000);
        assert_eq!(position.y_raw, 2000);
        assert_eq!(position.z_raw, z_raw);
        assert_eq!(
    ...
    fn exact_placeable_add_identity_resolves_by_surrounding_position_output_equivalence() {
        ...
        fragment_bits.extend([
            ...
            false, // no optional object id bytes.
            false, // static/plot stays packet-authored.
            false, // useable conflicts with both equivalent module rows.
            false, // trap disarmable already matches.
            false, // lockable conflicts with both equivalent module rows.
            false, // locked already matches.
            false, // unknown 0x1AC sibling stays packet-authored.
        ...
        fragment_bits.extend([(z_raw & 0b10) != 0, (z_raw & 0b01) != 0]);
        let mut payload = live_object_payload_from_parts(&live, &fragment_bits)
            .expect("exact U/09, A/09, U/09 surrounding-position payload");
        ...
        .expect("output-equivalent surrounding positions should rewrite the A/09 add");
        assert_eq!(summary.add_records_examined, 1);
        assert_eq!(summary.update_records_examined, 2);
        assert_eq!(summary.add_records_rewritten, 1);
        assert_eq!(summary.update_records_rewritten, 0);
        ...
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_surrounding_position_equivalence, 1,
            "different surrounding positions are safe only because their add-visible output matches"
        );
        assert_eq!(
        ...
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_following_position, 0,
            "bracketing evidence should not be reported as a one-sided following-position rewrite"
        );
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_preceding_position, 0,
            "bracketing evidence should not be reported as a one-sided preceding-position rewrite"
        );
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_add_output_equivalence, 0,
            "the surrounding-position rule should resolve before the broader add-output fallback"
        );
        assert_eq!(
            summary.exact_placeable_update_identity_resolved_by_position,
            2
        ...
        let claim =
            claim_payload_if_verified(&payload).expect("post-rewrite exact U/09, A/09, U/09 claim");
        assert_eq!(claim.mentions.len(), 3);
        assert_eq!(claim.mentions[1].record_offset, add_offset);
    ...
    #[test]
    fn exact_placeable_add_surrounding_position_conflict_stays_identity_blocked() {
        let object_id = 0x8000_3618u32;
        let z_raw = encode_ee_position_z(0.0).expect("test z should encode");
        let following_update_mask = LEGACY_UPDATE_POSITION_MASK | LEGACY_UPDATE_STATE_MASK;
        ...
        live.extend_from_slice(&[b'U', PLACEABLE_OBJECT_TYPE]);
        live.extend_from_slice(&object_id.to_le_bytes());
        live.extend_from_slice(&following_update_mask.to_le_bytes());
        live.extend_from_slice(&3000u16.to_le_bytes());
        live.extend_from_slice(&4000u16.to_le_bytes());
        live.extend_from_slice(&((z_raw >> 2) as u16).to_le_bytes());
        ...
        fragment_bits.extend([
            ...
            false, // no optional object id bytes.
            false, // static/plot stays packet-authored.
            false, // useable conflicts with both possible add targets.
            false, // trap disarmable already matches.
            false, // lockable conflicts with both possible add targets.
            false, // locked already matches.
            false, // unknown 0x1AC sibling stays packet-authored.
            true,  // name-valid stays packet-authored.
            false, // EE-only light/visual guard before the transform map.
        ...
        .expect("following U/09 state rewrite should expose the add conflict diagnostics");
        assert_eq!(summary.add_records_examined, 1);
        assert_eq!(summary.update_records_examined, 2);
        assert_eq!(summary.add_records_rewritten, 0);
        assert_eq!(summary.update_records_rewritten, 1);
        assert_eq!(summary.exact_placeable_add_unique_targets, 0);
        assert_eq!(summary.exact_placeable_add_identity_blocked, 1);
        assert_eq!(
            summary.exact_placeable_add_identity_surrounding_position_conflicts, 1,
            "the add must not silently choose either side of conflicting bracketing position proof"
        );
        assert_eq!(
            summary.exact_placeable_add_identity_surrounding_position_conflict_output_unavailable,
            0,
            "both bracketing rows have enough module output proof in this case"
        );
        assert_eq!(
            summary
                .exact_placeable_add_identity_surrounding_position_conflict_output_missing_template_resref_rows,
            0,
            "a true output divergence must not be classified as missing TemplateResRef proof"
        );
        assert_eq!(
            summary.exact_placeable_add_identity_surrounding_position_conflict_output_divergent, 1,
            "the conflict is a real add-output divergence, not missing TemplateResRef proof"
        );
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_surrounding_position,
            0
        ...
        let claim =
            claim_payload_if_verified(&payload).expect("post-rewrite exact conflicting claim");
        assert_eq!(claim.mentions.len(), 3);
        assert_eq!(claim.mentions[1].record_offset, add_offset);
        assert_eq!(
            claim.mentions[1].placeable_appearance,
            ...
            "the conflicting add remains packet-authored"
        );
        assert_eq!(
            claim.mentions[1].placeable_state,
            Some(LiveObjectPlaceableState {
        ...
        assert_eq!(
            claim.mentions[2].placeable_state,
            Some(LiveObjectPlaceableState {
                lockable: Some(true),
                locked: Some(false),
                ..LiveObjectPlaceableState::default()
            }),
            "the following U/09 still rewrites its own parser-owned state"
        );
    }

    #[test]
    fn exact_placeable_surrounding_position_conflict_classifies_missing_output() {
        ...
        let normal_row = AreaPlaceableContextRow {
            object_id: 0x8000_3619,
            appearance: 0x0033,
            module_state: Some(module_state),
            ..AreaPlaceableContextRow::default()
        };
        let custom_missing_resref_row = AreaPlaceableContextRow {
            object_id: 0x8000_3619,
            appearance: 0xFFFE,
            module_state: Some(module_state),
            module_template_resref: None,
        ...
        match surrounding_update_position_identity_match_for_add(
            AddPositionIdentityMatch::Unique(&normal_row),
            AddPositionIdentityMatch::Unique(&custom_missing_resref_row),
        ) {
            SurroundingAddPositionIdentityMatch::Conflict {
                output_unavailable,
                output_missing_template_resref_rows,
                output_divergent,
            } => {
                assert!(output_unavailable);
                assert_eq!(output_missing_template_resref_rows, 1);
                assert!(!output_divergent);
            }
            other => panic!("expected missing-output surrounding conflict, got {other:?}"),
        }
    }

    #[test]
    fn exact_placeable_surrounding_position_fixed_output_equivalence_rewrites_state_only() {
        let object_id = 0x8000_361Au32;
        let z_raw = encode_ee_position_z(0.0).expect("test z should encode");
        ...
        fragment_bits.extend([
            ...
            false, // no optional object id bytes.
            false, // static/plot stays packet-authored.
            false, // useable conflicts with both fixed-output-equivalent rows.
            false, // trap disarmable already matches.
            false, // lockable conflicts with both fixed-output-equivalent rows.
            false, // locked already matches.
            false, // unknown 0x1AC sibling stays packet-authored.
            true,  // name-valid stays packet-authored.
            false, // EE-only light/visual guard before the transform map.
        ...
        fragment_bits.extend([(z_raw & 0b10) != 0, (z_raw & 0b01) != 0]);
        let mut payload = live_object_payload_from_parts(&live, &fragment_bits)
            .expect("exact U/09, custom A/09, U/09 surrounding-position payload");
        ...
        let area_context = crate::translate::area::AreaPlaceableContext {
            ...
            static_rows: vec![
                crate::translate::area::AreaPlaceableContextRow {
                    ...
                    z: 0.0,
                    module_state: Some(module_state),
                    module_template_resref: Some(*b"plc_fixed_one\0\0\0"),
                    ..crate::translate::area::AreaPlaceableContextRow::default()
                },
                crate::translate::area::AreaPlaceableContextRow {
                    object_id,
                    ...
                    z: 0.0,
                    module_state: Some(module_state),
                    module_template_resref: Some(*b"plc_fixed_two\0\0\0"),
                    ..crate::translate::area::AreaPlaceableContextRow::default()
                },
            ],
            ..crate::translate::area::AreaPlaceableContext::default()
        ...
        .expect("fixed-output-equivalent surrounding positions should rewrite A/09 state only");
        assert_eq!(summary.add_records_examined, 1);
        assert_eq!(summary.update_records_examined, 2);
        assert_eq!(summary.add_records_rewritten, 1);
        assert_eq!(summary.update_records_rewritten, 0);
        ...
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_surrounding_position_equivalence,
            1
        );
        assert_eq!(
            summary
        ...
        assert_eq!(
            summary.exact_placeable_add_identity_surrounding_position_conflict_output_divergent,
            1
        );
        assert_eq!(
            summary
                .exact_placeable_add_module_custom_template_resref_fixed_width_synthesized_update,
            0,
            "fixed-output equivalence must not choose either divergent custom carrier"
        );
        assert_eq!(summary.exact_placeable_add_appearance_rewritten, 0);
        assert_eq!(summary.exact_placeable_add_state_rewritten, 1);

        let claim = claim_payload_if_verified(&payload)
            .expect("post-rewrite exact U/09, custom A/09, U/09 claim");
        assert_eq!(claim.mentions.len(), 3);
        assert_eq!(claim.mentions[1].record_offset, add_offset);
        assert_eq!(claim.mentions[2].record_offset, add_end);
        assert_eq!(
            ...
            "A/09 remains fixed-width and carries no TemplateResRef"
        );
        assert_eq!(
            claim.mentions[1].placeable_state,
            Some(LiveObjectPlaceableState {
    ...
    fn exact_placeable_add_identity_resolves_by_following_position_output_equivalence() {
        ...
        fragment_bits.extend([
            ...
            false, // no optional object id bytes.
            false, // static/plot stays packet-authored.
            false, // useable conflicts with the equivalent module rows.
            false, // trap disarmable already matches.
            false, // lockable conflicts with the equivalent module rows.
            false, // locked already matches.
            false, // unknown 0x1AC sibling stays packet-authored.
            true,  // name-valid stays packet-authored.
            false, // EE-only light/visual guard before the transform map.
        ...
        .expect("same-position equivalent rows should resolve the add output");
        assert_eq!(summary.add_records_examined, 1);
        assert_eq!(summary.update_records_examined, 1);
        assert_eq!(summary.add_records_rewritten, 1);
        assert_eq!(summary.update_records_rewritten, 0);
        ...
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_following_position_equivalence, 1,
            "the position proof is ambiguous, but all matching rows emit identical add output"
        );
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_add_output_equivalence, 0,
            "duplicate-confidence rows should not fall through to broad add-output equivalence"
        );
        assert_eq!(
            summary.exact_placeable_add_identity_blocked_following_position_ambiguous_matches, 0,
            "resolved position-equivalent rows are no longer an unresolved blocker"
        );
        assert_eq!(summary.exact_placeable_update_identity_blocked, 1);
        assert_eq!(summary.exact_placeable_add_appearance_rewritten, 1);
        assert_eq!(summary.exact_placeable_add_state_rewritten, 1);
    ...
    #[test]
    fn exact_placeable_add_following_position_fixed_output_equivalence_suppresses_carrier() {
        ...
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_following_position_equivalence, 0,
            "full custom-carrier output is still unproven"
        );
        assert_eq!(
            summary
        ...
        assert_eq!(
            summary
                .exact_placeable_add_module_custom_template_resref_fixed_width_synthesized_update,
            0,
            "fixed-output equivalence must not choose the one proven custom carrier"
        );
        assert_eq!(summary.exact_placeable_update_state_rewritten, 1);
        ...
        assert_eq!(
            ...
            "fixed-output equivalence cannot write a custom A/09 appearance without a proven carrier"
        );
        assert_eq!(
            claim.mentions[0].placeable_state,
            Some(LiveObjectPlaceableState {
            ...
            "A/09 state bits are safe because all same-position rows agree on fixed output"
        );
    ...
    #[test]
    fn exact_placeable_add_pre_add_custom_update_carrier_resolves_fixed_output_identity() {
        let object_id = 0x8000_3624u32;
        let target_resref = *b"plc_pre_carry\0\0\0";
        let z_raw = encode_ee_position_z(0.0).expect("test z should encode");
        let update_mask = LEGACY_UPDATE_POSITION_MASK | LEGACY_UPDATE_APPEARANCE_MASK;
        ...
        let mut payload = live_object_payload_from_parts(&live, &fragment_bits)
            .expect("custom U/09 carrier followed by fixed-width A/09 payload");
        ...
        .expect("pre-add custom U/09 should prove the add row and synthesize a following carrier");
        assert_eq!(summary.add_records_examined, 1);
        assert_eq!(summary.update_records_examined, 1);
        assert_eq!(summary.add_records_rewritten, 1);
        assert_eq!(summary.update_records_rewritten, 0);
        ...
        assert_eq!(
            ...
            "the pre-add U/09 owns both position and custom CResRef proof"
        );
        assert_eq!(
            summary
                .exact_placeable_add_identity_resolved_by_preceding_position_fixed_output_equivalence,
            0,
            "a matching pre-add custom carrier is full output proof, not fixed-output-only"
        );
        assert_eq!(
            summary.exact_placeable_add_module_custom_fixed_width_unproven_carrier_skipped,
            0
        ...
        assert_eq!(claim.mentions.len(), 3);
        assert_eq!(claim.mentions[1].record_offset, add_offset);
        assert_eq!(claim.mentions[1].record_end, add_end);
        assert_eq!(
            claim.mentions[0].placeable_appearance,
            Some(LiveObjectPlaceableAppearance {
                appearance: 0xFFFE,
        ...
        assert_eq!(
            ...
            "the synthesized following carrier restates the proven custom TemplateResRef after add"
        );
        assert_eq!(claim.mentions[2].record_offset, add_end);
        assert_eq!(
            claim.mentions[2].fragment_bit_start, claim.mentions[1].fragment_bit_end,
    ...
    fn exact_placeable_add_preceding_position_fixed_output_equivalence_suppresses_carrier() {
        ...
        let mut payload = live_object_payload_from_parts(&live, &fragment_bits)
            .expect("exact U/09 position followed by custom A/09 payload");
            ...
            static_rows: vec![
                ...
                crate::translate::area::AreaPlaceableContextRow {
                    ...
                    object_id_confidence:
                        crate::translate::area::AreaPlaceableContextObjectIdConfidence::DuplicateObjectId,
                    module_state: Some(module_state),
                    module_template_resref: Some(*b"plc_prev_res\0\0\0\0"),
                    ..crate::translate::area::AreaPlaceableContextRow::default()
                },
            ],
            ..crate::translate::area::AreaPlaceableContext::default()
        ...
        .expect("preceding position fixed-output equivalence should rewrite add state");
        assert_eq!(summary.add_records_examined, 1);
        assert_eq!(summary.update_records_examined, 1);
        assert_eq!(summary.add_records_rewritten, 1);
        assert_eq!(summary.update_records_rewritten, 0);
        ...
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_preceding_position_equivalence, 0,
            "full custom-carrier output is still unproven"
        );
        assert_eq!(
            summary
    ...
    fn exact_placeable_add_following_position_stops_at_lifecycle_delete() {
        ...
        .expect("following U/09 state rewrite should make diagnostics observable");
        assert_eq!(summary.add_records_examined, 1);
        assert_eq!(summary.update_records_examined, 1);
        assert_eq!(summary.add_records_rewritten, 0);
        assert_eq!(summary.update_records_rewritten, 1);
        ...
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_following_position, 0,
            "A/09 must not borrow a position proof across a same-object delete"
        );
        assert_eq!(
            summary.exact_placeable_add_identity_resolved_by_preceding_position, 0,
            "no preceding position proof should be inferred across the A/09 lifecycle start"
        );
        assert_eq!(
            summary.exact_placeable_add_identity_blocked_following_position_lifecycle_blocked,
            1
        ...
        assert_eq!(
            summary.exact_placeable_update_identity_resolved_by_position, 1,
            "the later U/09 still owns its own position proof after the lifecycle break"
        );
        assert_eq!(summary.exact_placeable_add_appearance_rewritten, 0);
        assert_eq!(summary.exact_placeable_add_state_rewritten, 0);
        assert_eq!(summary.exact_placeable_update_state_rewritten, 1);
        ...
        let claim =
            claim_payload_if_verified(&payload).expect("post-rewrite exact A/09, D/09, U/09 claim");
        assert_eq!(claim.mentions.len(), 3);
        assert_eq!(claim.mentions[1].opcode, b'D');
        assert_eq!(claim.mentions[2].record_offset, update_offset);
        assert_eq!(
            claim.mentions[0].placeable_appearance,
            Some(LiveObjectPlaceableAppearance {
                appearance: 0x0011,
            ...
            "the lifecycle-fenced add remains packet-authored"
        );
        assert_eq!(
            claim.mentions[2].placeable_state,
            Some(LiveObjectPlaceableState {
            ...
            "the later update can still reconcile its own parser-owned state"
        );
    }
}
...
pub struct LiveObjectUpdateRewriteFailure {
    ...
    pub kind: LiveObjectUpdateRewriteFailureKind,
    pub offset: usize,
...
impl LiveObjectUpdateRewriteFailureKind {
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            Self::ItemUpdateCursorAfterLedgerHandoff => {
                "item-update-cursor-failed-after-ledger-handoff"
            }
            Self::ItemUpdateCursorAfterUnownedLedgerGap => {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LiveObjectUpdateItemCursorGapOrigin {
    NotAfterFocusCursor,
    FocusPositionBits,
    PartialFocusPositionBits,
    FocusOrientationSelector,
    PartialFocusOrientationSelector,
    FocusOrientationVectorPrefixBits,
    FocusOrientationScalarPrefixBits,
    FocusStatePrefixBits,
    FocusNameOrLocstringPrefixBits,
    InsideFocusRowUnclassified,
}

impl LiveObjectUpdateItemCursorGapOrigin {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::NotAfterFocusCursor => "not-after-focus-cursor",
            Self::FocusPositionBits => "focus-position-bits",
            Self::PartialFocusPositionBits => "partial-focus-position-bits",
            Self::FocusOrientationSelector => "focus-orientation-selector",
            Self::PartialFocusOrientationSelector => "partial-focus-orientation-selector",
            Self::FocusOrientationVectorPrefixBits => "focus-orientation-vector-prefix-bits",
            Self::FocusOrientationScalarPrefixBits => "focus-orientation-scalar-prefix-bits",
            Self::FocusStatePrefixBits => "focus-state-prefix-bits",
            Self::FocusNameOrLocstringPrefixBits => "focus-name-or-locstring-prefix-bits",
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum LiveObjectUpdateItemCursorSourceOwner {
    NoLedger,
    BeforeLedgerRow,
    InsidePreviousRow,
    ContiguousTail,
    UnownedEmittedGap,
    UnownedSourceGap,
    UnownedEmittedAndSourceGap,
}

impl LiveObjectUpdateItemCursorSourceOwner {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::NoLedger => "no-ledger",
            Self::BeforeLedgerRow => "before-ledger-row",
            Self::InsidePreviousRow => "inside-previous-row",
            Self::ContiguousTail => "contiguous-tail",
...
pub(crate) fn try_get_legacy_short_door_add_record_end_for_transport(
    live_bytes: &[u8],
    offset: usize,
    scan_end: usize,
) -> Option<usize> {
    boundary::try_get_legacy_short_door_add_record_end_for_transport(live_bytes, offset, scan_end)
}
...
pub(crate) fn looks_like_legacy_item_add_record_boundary_for_transport(
    live_bytes: &[u8],
    offset: usize,
) -> bool {
    appearance::looks_like_legacy_item_add_record_boundary(live_bytes, offset)
        && !matches!(
...
pub(crate) fn try_get_legacy_item_add_record_end_for_transport(
    live_bytes: &[u8],
    offset: usize,
    scan_end: usize,
) -> Option<usize> {
...
fn verified_record_position(
    ...
    Some(LiveObjectRecordPosition {
        x_raw: fixed_x as u16,
        y_raw: fixed_y as u16,
        z_raw,
...
fn compact_ee_placeable_add_repair_claims_following_same_object_update(
    live_bytes: &[u8],
    offset: usize,
    record_end: usize,
    fragment_bits: &[bool],
    ...
    let mut exact_following_cursor = exact_cursor;
    if record::advance_verified_update_record_for_ee(
        live_bytes,
        record_end,
        following_end,
    ...
    if add_guard::repair_verified_ee_placeable_add_compact_source_bits(
        &candidate_live,
        offset,
        record_end,
        &mut candidate_bits,
...
fn legacy_door_add_repair_claims_following_same_object_update(
    ...
    if record_end >= live_bytes.len()
        ...
        || live_bytes.get(record_end).copied() != Some(b'U')
        || live_bytes.get(record_end + 1).copied() != Some(DOOR_OBJECT_TYPE)
    {
        return false;
    }
...
fn rewrite_update_records_payload_with_area_context_inner(
    ...
    while offset + 2 <= live_bytes.len() {
        ...
        if opcode == b'A' {
            ...
            if bit_cursor_reliable {
                if legacy_door_add_repair_claims_following_same_object_update(
                    &live_bytes,
                    offset,
                    record_end,
                    &fragment_bits,
                        ...
                        )
                    {
                        ...
                        summary.bytes_inserted = summary
                            .bytes_inserted
                            .saturating_add(add_rewrite.bytes_inserted);
                        summary.bytes_removed =
                            summary.bytes_removed.saturating_add(add_rewrite.bytes_removed);
                ...
                if compact_ee_placeable_add_repair_claims_following_same_object_update(
                    &live_bytes,
                    offset,
                    record_end,
                    &fragment_bits,
...
fn rewrite_verified_placeable_states_with_area_context_if_possible(
    ...
    let mut live_bytes =
        payload[HIGH_LEVEL_HEADER_BYTES + CNW_LENGTH_BYTES..claim.declared].to_vec();
    ...
    for mention in &claim.mentions {
        ...
        match mention.opcode {
            b'A' => {
                ...
                let (base_target, area_rows) = placeable_static_reconciliation_target_for_object(
                    area_context,
                    mention.object_id,
                );
                ...
                let selection = placeable_static_reconciliation_selection_for_add(
                    area_context,
                    mention.object_id,
                    mention.record_offset,
                    mention.record_end,
                ...
                if selection.identity_resolved_by_following_position_equivalence {
                    summary
                ...
                if selection.identity_resolved_by_following_position_fixed_output_equivalence {
                    summary
                        .exact_placeable_add_identity_resolved_by_following_position_fixed_output_equivalence =
                        summary
                            .exact_placeable_add_identity_resolved_by_following_position_fixed_output_equivalence
                ...
                if selection.identity_resolved_by_preceding_position_equivalence {
                    summary
                ...
                if selection.identity_resolved_by_preceding_position_fixed_output_equivalence {
                    summary
                        .exact_placeable_add_identity_resolved_by_preceding_position_fixed_output_equivalence =
                        summary
                            .exact_placeable_add_identity_resolved_by_preceding_position_fixed_output_equivalence
                ...
                if selection.identity_resolved_by_surrounding_position_equivalence {
                    summary
                ...
                if selection.identity_surrounding_position_conflict_output_unavailable {
                    summary
                ...
                record_exact_placeable_identity_blocked_add_following_position_match(
                    &mut summary,
                    target,
                    area_context,
                    mention.object_id,
                    ...
                    && is_custom_placeable_appearance(row.appearance)
                {
                    ...
                    if selection.custom_appearance_carrier_unproven() {
                        ...
                        tracing::debug!(
                            ...
                            record_offset,
                            record_end,
                            area_appearance = format_args!("0x{:04X}", row.appearance),
                            ...
                            area_static_identity_surrounding_position_conflict_output_unavailable =
                                selection.identity_surrounding_position_conflict_output_unavailable,
                            area_static_identity_surrounding_position_conflict_output_missing_template_resref_rows =
                                selection
                                    .identity_surrounding_position_conflict_output_missing_template_resref_rows,
...
struct PlaceableStaticReconciliationSelection<'a> {
    ...
    identity_resolved_by_preceding_position: bool,
    identity_resolved_by_preceding_position_equivalence: bool,
    identity_resolved_by_preceding_position_fixed_output_equivalence: bool,
    identity_resolved_by_surrounding_position: bool,
    identity_resolved_by_surrounding_position_equivalence: bool,
    identity_surrounding_position_conflict: bool,
    identity_surrounding_position_conflict_output_unavailable: bool,
    identity_surrounding_position_conflict_output_missing_template_resref_rows: u32,
    identity_surrounding_position_conflict_output_divergent: bool,
    identity_resolved_by_add_output_equivalence: bool,
}
...
fn placeable_static_reconciliation_selection_for_add<'a>(
    ...
    let mut surrounding_position_conflict = false;
    let mut surrounding_position_conflict_output_unavailable = false;
    let mut surrounding_position_conflict_output_missing_template_resref_rows = 0u32;
    let mut surrounding_position_conflict_output_divergent = false;
    if matches!(
        target,
        ...
        let following_position_match = following_update_position_identity_match_for_add(
            area_context,
            object_id,
            add_record_end,
            claim_summary,
        ...
        match surrounding_update_position_identity_match_for_add(
            following_position_match,
            preceding_position_match,
        ) {
            SurroundingAddPositionIdentityMatch::Agreed {
                row,
                output_equivalence,
            } => {
                return PlaceableStaticReconciliationSelection {
                    ...
                    identity_resolved_by_preceding_position_equivalence: false,
                    identity_resolved_by_preceding_position_fixed_output_equivalence: false,
                    identity_resolved_by_surrounding_position: true,
                    identity_resolved_by_surrounding_position_equivalence: output_equivalence,
                    identity_surrounding_position_conflict: false,
                    identity_surrounding_position_conflict_output_unavailable: false,
                    identity_surrounding_position_conflict_output_missing_template_resref_rows: 0,
                    identity_surrounding_position_conflict_output_divergent: false,
            ...
            SurroundingAddPositionIdentityMatch::FixedOutputEquivalent {
                row,
                output_missing_template_resref_rows,
                output_divergent,
            } => {
                return PlaceableStaticReconciliationSelection {
                    ...
                    identity_resolved_by_preceding_position_fixed_output_equivalence: false,
                    identity_resolved_by_surrounding_position: true,
                    identity_resolved_by_surrounding_position_equivalence: true,
                    identity_surrounding_position_conflict: true,
                    identity_surrounding_position_conflict_output_unavailable:
                        output_missing_template_resref_rows != 0,
                    identity_surrounding_position_conflict_output_missing_template_resref_rows:
                        output_missing_template_resref_rows,
                    identity_surrounding_position_conflict_output_divergent: output_divergent,
                    identity_resolved_by_add_output_equivalence: false,
                };
            }
            SurroundingAddPositionIdentityMatch::Conflict {
                ...
                surrounding_position_conflict = true;
                surrounding_position_conflict_output_unavailable = output_unavailable;
                surrounding_position_conflict_output_missing_template_resref_rows =
                    output_missing_template_resref_rows;
                surrounding_position_conflict_output_divergent = output_divergent;
            }
            SurroundingAddPositionIdentityMatch::MissingResolvedSide => {
                match following_position_match {
                    AddPositionIdentityMatch::Unique(row) => {
                        return PlaceableStaticReconciliationSelection {
                            target:
                    ...
                    AddPositionIdentityMatch::EquivalentOutput(row) => {
                        return PlaceableStaticReconciliationSelection {
                            target:
                                AreaPlaceableContextStaticReconciliationTarget::UniqueModuleBacked(
                                    row,
                    ...
                    AddPositionIdentityMatch::FixedOutputEquivalent { row, .. } => {
                        return PlaceableStaticReconciliationSelection {
                            target:
                                AreaPlaceableContextStaticReconciliationTarget::UniqueModuleBacked(
                                    row,
                ...
                match preceding_position_match {
                    AddPositionIdentityMatch::Unique(row) => {
                        return PlaceableStaticReconciliationSelection {
                            target:
                                AreaPlaceableContextStaticReconciliationTarget::UniqueModuleBacked(
                    ...
                    AddPositionIdentityMatch::FixedOutputEquivalent { row, .. } => {
                        return PlaceableStaticReconciliationSelection {
                            ...
                            identity_resolved_by_preceding_position: true,
                            identity_resolved_by_preceding_position_equivalence: false,
                            identity_resolved_by_preceding_position_fixed_output_equivalence: true,
                            identity_resolved_by_surrounding_position: false,
                            identity_resolved_by_surrounding_position_equivalence: false,
                            identity_surrounding_position_conflict: false,
                            identity_surrounding_position_conflict_output_unavailable: false,
        ...
        if let Some(row) =
            equivalent_module_static_row_for_verified_add_output(area_context, object_id)
        {
            return PlaceableStaticReconciliationSelection {
                target: AreaPlaceableContextStaticReconciliationTarget::UniqueModuleBacked(row),
                identity_resolved_by_position: false,
                ...
                identity_resolved_by_surrounding_position: false,
                identity_resolved_by_surrounding_position_equivalence: false,
                identity_surrounding_position_conflict: surrounding_position_conflict,
                identity_surrounding_position_conflict_output_unavailable:
                    surrounding_position_conflict_output_unavailable,
                identity_surrounding_position_conflict_output_missing_template_resref_rows:
                    surrounding_position_conflict_output_missing_template_resref_rows,
                identity_surrounding_position_conflict_output_divergent:
                    surrounding_position_conflict_output_divergent,
                identity_resolved_by_add_output_equivalence: true,
            };
        }
    }
...
fn record_exact_placeable_identity_blocked_add_following_position_match(
    summary: &mut LiveObjectUpdateRewriteSummary,
    target: AreaPlaceableContextStaticReconciliationTarget<'_>,
    area_context: &AreaPlaceableContext,
    object_id: u32,
    ...
    match following_update_position_identity_match_for_add(
        area_context,
        object_id,
        add_record_end,
        claim_summary,
    ) {
        AddPositionIdentityMatch::Unique(_)
        | AddPositionIdentityMatch::EquivalentOutput(_)
        | AddPositionIdentityMatch::FixedOutputEquivalent { .. } => {}
        AddPositionIdentityMatch::MissingBeforeLifecycle => {
            summary.exact_placeable_add_identity_blocked_following_position_missing = summary
...
#[derive(Debug, Clone, Copy)]
enum AddPositionIdentityMatch<'a> {
    Unique(&'a AreaPlaceableContextRow),
    EquivalentOutput(&'a AreaPlaceableContextRow),
    FixedOutputEquivalent {
        row: &'a AreaPlaceableContextRow,
...
#[derive(Debug, Clone, Copy)]
struct AddPositionSelectedRow<'a> {
    row: &'a AreaPlaceableContextRow,
    output_equivalence: bool,
    fixed_output_equivalence: bool,
    output_missing_template_resref_rows: u32,
    output_divergent: bool,
}

impl<'a> AddPositionIdentityMatch<'a> {
    fn selected_row(self) -> Option<AddPositionSelectedRow<'a>> {
        match self {
            AddPositionIdentityMatch::Unique(row) => Some(AddPositionSelectedRow {
                row,
                output_equivalence: false,
                fixed_output_equivalence: false,
                output_missing_template_resref_rows: u32::from(
                    placeable_add_output_missing_template_resref_for_module_static_row(row),
                ),
                output_divergent: false,
            }),
            AddPositionIdentityMatch::EquivalentOutput(row) => Some(AddPositionSelectedRow {
                row,
            ...
            AddPositionIdentityMatch::FixedOutputEquivalent {
                ...
                output_missing_template_resref_rows,
                output_divergent,
            } => Some(AddPositionSelectedRow {
                row,
                output_equivalence: true,
                fixed_output_equivalence: true,
                output_missing_template_resref_rows,
                output_divergent,
            }),
            AddPositionIdentityMatch::MissingBeforeLifecycle
...
#[derive(Debug, Clone, Copy)]
enum SurroundingAddPositionIdentityMatch<'a> {
    MissingResolvedSide,
    Agreed {
        row: &'a AreaPlaceableContextRow,
        output_equivalence: bool,
...
fn surrounding_update_position_identity_match_for_add<'a>(
    following: AddPositionIdentityMatch<'a>,
    preceding: AddPositionIdentityMatch<'a>,
) -> SurroundingAddPositionIdentityMatch<'a> {
    let Some(following) = following.selected_row() else {
        return SurroundingAddPositionIdentityMatch::MissingResolvedSide;
    };
    let Some(preceding) = preceding.selected_row() else {
        return SurroundingAddPositionIdentityMatch::MissingResolvedSide;
    };

    let fixed_output_only =
        following.fixed_output_equivalence || preceding.fixed_output_equivalence;
    let missing_template_resref_rows = following
        .output_missing_template_resref_rows
        .saturating_add(preceding.output_missing_template_resref_rows);
    let candidate_output_divergent = following.output_divergent || preceding.output_divergent;

    if std::ptr::eq(following.row, preceding.row) && !fixed_output_only {
        return SurroundingAddPositionIdentityMatch::Agreed {
            row: following.row,
            output_equivalence: following.output_equivalence || preceding.output_equivalence,
        };
    }

    let following_output = placeable_add_output_for_module_static_row(following.row);
    let preceding_output = placeable_add_output_for_module_static_row(preceding.row);
    if !fixed_output_only {
        if let (Some(following_output), Some(preceding_output)) =
            (following_output, preceding_output)
            && following_output == preceding_output
        {
            return SurroundingAddPositionIdentityMatch::Agreed {
                row: following.row,
    ...
    let output_divergent = candidate_output_divergent
        || (following_output.is_some()
            && preceding_output.is_some()
            && following_output != preceding_output);
    if let (Some(following_fixed_output), Some(preceding_fixed_output)) = (
        placeable_add_fixed_output_for_module_static_row(following.row),
        placeable_add_fixed_output_for_module_static_row(preceding.row),
    ) && following_fixed_output == preceding_fixed_output
    {
        return SurroundingAddPositionIdentityMatch::FixedOutputEquivalent {
            row: following.row,
            output_missing_template_resref_rows: missing_template_resref_rows,
            output_divergent,
        };
    }

    match (following_output, preceding_output) {
        (Some(_), Some(_)) => SurroundingAddPositionIdentityMatch::Conflict {
            output_unavailable: false,
            output_missing_template_resref_rows: missing_template_resref_rows,
            output_divergent,
        },
        _ => SurroundingAddPositionIdentityMatch::Conflict {
        ...
        match mention.opcode {
            b'U' => {
                if let Some(position) = mention.position {
                    following_position = Some(position);
                    break;
                }
            }
            b'A' | b'D' => return AddPositionIdentityMatch::LifecycleBlocked,
            _ => {}
        }
    }

    let Some(position) = following_position else {
        return AddPositionIdentityMatch::MissingBeforeLifecycle;
    };
    add_position_identity_match_from_raw_position(
        area_context,
        object_id,
        position.x_raw,
        position.y_raw,
...
fn preceding_update_custom_carrier_identity_match_for_add<'a>(
    area_context: &'a AreaPlaceableContext,
    object_id: u32,
    add_record_offset: usize,
    claim_summary: &LiveObjectUpdateClaimSummary,
...
fn preceding_update_position_identity_match_for_add<'a>(
    ...
    // The position bits come only from an exact-validated U/09 mention. Same-
    // object A/D rows fence the search because they start or end a lifecycle.
    let mut preceding_position = None;
    for mention in claim_summary.mentions.iter().rev() {
        if mention.record_offset >= add_record_offset {
            continue;
        }
        ...
        match mention.opcode {
            b'U' => {
                if let Some(position) = mention.position {
                    preceding_position = Some(position);
                    break;
                }
            }
            b'A' | b'D' => return AddPositionIdentityMatch::LifecycleBlocked,
    ...
    let Some(position) = preceding_position else {
        return AddPositionIdentityMatch::MissingBeforeLifecycle;
    };
    add_position_identity_match_from_raw_position(
        area_context,
...
fn add_position_identity_match_from_raw_position<'a>(
    area_context: &'a AreaPlaceableContext,
    object_id: u32,
    x_raw: u16,
    y_raw: u16,
    ...
    let matched =
        module_static_rows_matching_raw_position(area_context, object_id, x_raw, y_raw, z_raw);
    match matched.rows {
        0 => AddPositionIdentityMatch::NoStaticPositionMatch,
        1 => matched
            .selected
            .map(AddPositionIdentityMatch::Unique)
            .unwrap_or(AddPositionIdentityMatch::NoStaticPositionMatch),
        rows => {
            if let Some(row) = equivalent_module_static_row_matching_raw_position_output(
                area_context,
                object_id,
                x_raw,
                y_raw,
                ...
                AddPositionIdentityMatch::EquivalentOutput(row)
            } else if let Some(fixed_output) =
                equivalent_module_static_row_matching_raw_position_fixed_output(
                    area_context,
                    object_id,
                    x_raw,
                    y_raw,
            ...
            {
                AddPositionIdentityMatch::FixedOutputEquivalent {
                    row: fixed_output.row,
                    output_missing_template_resref_rows: fixed_output
                        .output_missing_template_resref_rows,
                    output_divergent: fixed_output.output_divergent,
                }
            } else {
                AddPositionIdentityMatch::AmbiguousStaticPositionMatch {
                    rows,
...
#[derive(Debug, Clone, Copy)]
struct FixedOutputEquivalentModuleStaticRow<'a> {
    row: &'a AreaPlaceableContextRow,
    output_missing_template_resref_rows: u32,
    output_divergent: bool,
}
...
fn equivalent_module_static_row_matching_raw_position_output<'a>(
    ...
    // A nearby U/09 position branch can narrow an identity-blocked A/09 to a
    // same-position static candidate set. If every candidate would emit the
    // same fixed add WORD/state bits and optional synthetic custom carrier, any
    // row in that set is output-equivalent for the add.
    let overlap = area_context.placeable_overlap_by(|row_object_id| {
        object_ids::equivalent_legacy_external_object_ids(row_object_id, object_id)
    });
    let mut selected = None;
...
fn equivalent_module_static_row_matching_raw_position_fixed_output<'a>(
    ...
    // This is narrower than full output-equivalence: it proves only the fields
    // owned by the fixed-width A/09 add row. A custom TemplateResRef carrier is
    // still unproven and must not be synthesized from this selection.
    let overlap = area_context.placeable_overlap_by(|row_object_id| {
        object_ids::equivalent_legacy_external_object_ids(row_object_id, object_id)
    });
    let mut selected = None;
...
fn module_static_row_matches_raw_position(
    ...
    area_position.x_raw == x_raw && area_position.y_raw == y_raw && area_position.z_raw == z_raw
}

fn unique_module_static_row_matching_raw_position(
    area_context: &AreaPlaceableContext,
    object_id: u32,
    x_raw: u16,
    y_raw: u16,
    ...
    let matched =
        module_static_rows_matching_raw_position(area_context, object_id, x_raw, y_raw, z_raw);
    (matched.rows == 1).then_some(matched.selected).flatten()
}
...
fn unique_module_static_row_matching_verified_update_position(
    ...
    unique_module_static_row_matching_raw_position(
        area_context,
        object_id,
...
fn unique_module_static_row_matching_verified_add_fixed_fields<'a>(
    area_context: &'a AreaPlaceableContext,
    object_id: u32,
    claim: VerifiedPlaceableAddExactClaim,
) -> Option<&'a AreaPlaceableContextRow> {
...
struct PlaceableAddOutput {
    ...
    custom_resref: Option<[u8; EE_UPDATE_APPEARANCE_RESREF_READ_BYTES]>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct PlaceableAddFixedOutput {
    appearance: u16,
    module_state: AreaPlaceableContextState,
}

fn equivalent_module_static_row_for_verified_add_output(
    area_context: &AreaPlaceableContext,
    object_id: u32,
) -> Option<&AreaPlaceableContextRow> {
    // A/09 owns only a fixed appearance WORD plus its add-state BOOL block.
    // Position and custom TemplateResRef are not present in the add record, so
    // identity-blocked rows are writable only when every module-backed static
    // candidate would produce byte/bit-identical add output and, for custom
    // appearances, an identical synthetic U/09 carrier.
    let overlap = area_context.placeable_overlap_by(|row_object_id| {
        object_ids::equivalent_legacy_external_object_ids(row_object_id, object_id)
    });
    let mut selected = None;
    ...
    if static_rows != 0 { selected } else { None }
}

fn placeable_add_output_for_module_static_row(
    row: &AreaPlaceableContextRow,
) -> Option<PlaceableAddOutput> {
    let module_state = row.module_state?;
    let custom_resref = if is_custom_placeable_appearance(row.appearance) {
        Some(row.module_template_resref?)
    } else {
        None
    };
    Some(PlaceableAddOutput {
        appearance: row.appearance,
        module_state,
        custom_resref,
    })
...
fn placeable_add_fixed_output_for_module_static_row(
    row: &AreaPlaceableContextRow,
) -> Option<PlaceableAddFixedOutput> {
    Some(PlaceableAddFixedOutput {
        appearance: row.appearance,
        module_state: row.module_state?,
    })
}

fn placeable_add_output_missing_template_resref_for_module_static_row(
    row: &AreaPlaceableContextRow,
) -> bool {
    is_custom_placeable_appearance(row.appearance) && row.module_template_resref.is_none()
}
...
fn trace_exact_placeable_reconciliation_summary(
    ...
    tracing::debug!(
        ...
        add_identity_surrounding_position_conflict_output_unavailable = summary
            .exact_placeable_add_identity_surrounding_position_conflict_output_unavailable,
        add_identity_surrounding_position_conflict_output_missing_template_resref_rows = summary
            .exact_placeable_add_identity_surrounding_position_conflict_output_missing_template_resref_rows,
        add_identity_surrounding_position_conflict_output_divergent = summary
            .exact_placeable_add_identity_surrounding_position_conflict_output_divergent,
        add_identity_resolved_by_add_output_equivalence =
            summary.exact_placeable_add_identity_resolved_by_add_output_equivalence,
        ...
        add_identity_blocked_following_position_lifecycle_blocked =
            summary.exact_placeable_add_identity_blocked_following_position_lifecycle_blocked,
        add_identity_blocked_following_position_no_static_match =
            summary.exact_placeable_add_identity_blocked_following_position_no_static_match,
        add_identity_blocked_following_position_ambiguous_matches =
            summary.exact_placeable_add_identity_blocked_following_position_ambiguous_matches,
...
fn reconcile_verified_placeable_add_appearance_with_area_context(
    ...
    tracing::info!(
        ...
        area_static_identity_resolved_by_fixed_fields =
            selection.identity_resolved_by_fixed_fields,
        area_static_identity_resolved_by_fixed_field_equivalence =
            selection.identity_resolved_by_fixed_field_equivalence,
        area_static_identity_resolved_by_following_position =
            selection.identity_resolved_by_following_position,
        area_static_identity_resolved_by_following_position_equivalence =
            selection.identity_resolved_by_following_position_equivalence,
        area_static_identity_resolved_by_following_position_fixed_output_equivalence =
            selection.identity_resolved_by_following_position_fixed_output_equivalence,
        area_static_identity_resolved_by_preceding_position =
            selection.identity_resolved_by_preceding_position,
        area_static_identity_resolved_by_preceding_position_equivalence =
            selection.identity_resolved_by_preceding_position_equivalence,
        area_static_identity_resolved_by_preceding_position_fixed_output_equivalence =
            selection.identity_resolved_by_preceding_position_fixed_output_equivalence,
        area_static_identity_resolved_by_surrounding_position =
            selection.identity_resolved_by_surrounding_position,
        area_static_identity_resolved_by_surrounding_position_equivalence =
            selection.identity_resolved_by_surrounding_position_equivalence,
        area_static_identity_surrounding_position_conflict =
            selection.identity_surrounding_position_conflict,
        area_static_identity_surrounding_position_conflict_output_unavailable =
            selection.identity_surrounding_position_conflict_output_unavailable,
        area_static_identity_surrounding_position_conflict_output_missing_template_resref_rows =
            selection.identity_surrounding_position_conflict_output_missing_template_resref_rows,
        area_static_identity_surrounding_position_conflict_output_divergent =
            selection.identity_surrounding_position_conflict_output_divergent,
        area_static_identity_resolved_by_add_output_equivalence =
            selection.identity_resolved_by_add_output_equivalence,
        area_resref = area_context.area_resref.as_str(),
        area_rows = %area_rows,
...
impl LiveObjectRewriteBitLedger {
    ...
    fn gap_before_cursor(&self, cursor: usize) -> Option<LiveObjectRewriteBitLedgerCursorGap> {
        ...
        let (emitted_gap_bit_start, emitted_gap_bit_end) = if cursor > previous.emitted_bit_end {
            (previous.emitted_bit_end, cursor)
        } else {
            (cursor, cursor)
        };
        let (source_gap_bit_start, source_gap_bit_end) =
            if implied_source_cursor > previous.source_bit_end {
                (previous.source_bit_end, implied_source_cursor)
            } else {
                (implied_source_cursor, implied_source_cursor)
            };
...
fn live_object_item_update_focus_gap_origin(
    ...
    if candidate_bit_start <= focus_bit_cursor {
        return LiveObjectUpdateItemCursorGapOrigin::NotAfterFocusCursor;
    }
    ...
    if (translated_mask & LEGACY_UPDATE_STATE_MASK) != 0 {
        ...
        if candidate_bit_start <= state_end {
            return LiveObjectUpdateItemCursorGapOrigin::FocusStatePrefixBits;
        }
    }

    if (translated_mask & LEGACY_UPDATE_NAME_MASK) != 0 {
        return LiveObjectUpdateItemCursorGapOrigin::FocusNameOrLocstringPrefixBits;
    }
        ...
        .find(|(_, row)| row.offset == focus_offset)
    {
        ...
        for neighbor in live_object_source_window_item_neighbor_cursor_claims(
            ...
            let ledger_source_gap_bits = ledger_gap
                ...
                .unwrap_or_else(|| "none".to_string());
            let ledger_emitted_gap_range = ledger_gap
                .map(|gap| format!("{}..{}", gap.emitted_gap_bit_start, gap.emitted_gap_bit_end))
                .unwrap_or_else(|| "none".to_string());
            let ledger_source_gap_range = ledger_gap
                .map(|gap| format!("{}..{}", gap.source_gap_bit_start, gap.source_gap_bit_end))
                .unwrap_or_else(|| "none".to_string());
            let ledger_emitted_gap_values = ledger_gap
                .map(|gap| {
                    format!(
                        "{:?}",
                        rewrite_bit_ledger.emitted_gap_values(fragment_bits, &gap)
                ...
                .unwrap_or_else(|| "none".to_string());
            let ledger_source_gap_values = ledger_gap
                .map(|gap| {
                    format!(
                        "{:?}",
                        rewrite_bit_ledger.source_gap_values(fragment_bits, &gap)
            ...
            eprintln!(
                ...
                ledger_emitted_gap_values,
                ledger_source_relation,
                ledger_source_gap_bits,
                ledger_source_gap_range,
...
fn write_u16_le(bytes: &mut [u8], offset: usize, value: u16) -> Option<()> {
    bytes
