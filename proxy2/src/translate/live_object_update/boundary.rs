...
pub(super) fn find_next_legacy_live_object_sub_message_boundary_after(
    ...
    if bytes.get(offset).copied() == Some(b'U') && bytes.get(offset + 1).copied() == Some(0x05) {
        // Top-level Diamond and EE `U/5` dispatch both read OBJECTID followed
        // by a DWORD update mask. The distinct visual-transform map belongs to
        // a counted nested `P/5` equipment `U` row, which has no type byte and
        // is owned by the appearance parser. Never shorten this top-level row
        // to an inferred one-byte selector shape.
