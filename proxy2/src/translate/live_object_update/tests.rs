...
#[test]
fn legacy_facing_zero_pre_rotates_for_ee_basis_correction() {
    assert_eq!(
...
fn translated_door_update_record_is_exactly_claimed() {
    ...
    live.push(super::writer::encode_ee_scalar_orientation_from_legacy_facing(0x2E00) as u8);
    ...
    let fragment_bits = vec![
        ...
        false, false, false, false, false, // EE scalar orientation branch + low bits.
        false, false, false, false, false, // five legacy door state bits.
        false, // EE-only neutral door state branch.
        false, // EE simple inline-name branch.
    ];
...
fn inventory_delta_followed_by_gui_item_add_rewrites_and_claims_exactly() {
    ...
    assert_eq!(claim.live_gui_fragment_bits, 6);
}
